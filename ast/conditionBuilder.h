struct ConditionBuilder {
    enum TYPE { ASSIGNMENT, STATEMENT } const type;
    ...
    struct Node {
        ...
        static constexpr uint8_t TYPE_PREFERENCE[TYPE::END_TARGET][2] = {
            {3, 1}, {3, 1}, {3, 1}, {3, 1}, {1, 3}, {1, 3}, {1, 3}, {1, 3}, {3, 3}, {3, 3},
            {3, 2}, {2, 3}, {},     {},     {3, 3}, {3, 3}, {3, 0}, {3, 0}, {0, 3}, {0, 3}};
    ...
    ConditionBuilder(
        const TYPE& type,
        Ast& ast,
        const uint32_t& endTargetLabel,
        const uint32_t& trueTargetLabel,
        const uint32_t& falseTargetLabel
    )
        : ast(ast), type(type) {
        if (type == ASSIGNMENT) {
    ...
    void add_node(
        ...
        const uint32_t& nodeLabel,
        const uint32_t& targetLabel,
        std::vector<Expression*>* const& expressions,
        const bool& hasAlternateTarget = false
    ) {
    ...
    bool link_nodes() {
        ...
        for (uint32_t i = conditionNodes.size(); i--;) {
            if (conditionNodes[i]->targetLabel == INVALID_ID)
                continue;

            for (uint32_t j = conditionNodes.size(); j--;) {
                if (conditionNodes[i]->targetLabel != conditionNodes[j]->nodeLabel)
                    continue;
            ...
            if (!conditionNodes[i]->targetNode)
                return false;
        }
        ...
        for (uint32_t i = conditionNodes.size() - 1; i--;) {
            switch (conditionNodes[i]->targetNode->type) {
                case Node::END_TARGET:
                    conditionNodes[i]->targetNode =
                        conditionNodes[i]->type == Node::TRUTHY_TEST ? trueTarget : falseTarget;
                ...
                case Node::TRUE_TARGET:
                    ...
                    continue;
                default:
                    if (Node::TYPE_PREFERENCE[conditionNodes[i]->type][conditionNodes[i]->inverted] != 3)
                        conditionNodes[i]->inverted = true;
                    continue;
            }
        }

        if (type != STATEMENT || conditionNodes[conditionNodes.size() - 2]->inverted)
            return;
            ...
            if (!conditionNodes[i]->incomingNodes) {
                if (conditionNodes[i - 1]->targetNode == conditionNodes[i]->targetNode) {
                    if (conditionNodes[i - 1]->inverted != conditionNodes[i]->inverted &&
                        !invert_any_node(conditionNodes[i - 1], conditionNodes[i]))
                        return false;
                    ...
                    conditionNodes[i]->hasAlternateTarget &&
                    (conditionNodes[i - 1]->targetNode == endAssignment || conditionNodes[i + 1] == endAssignment)) {
                    conditionNodes[i]->targetNode->incomingNodes--;
                    conditionNodes[i]->targetNode = endAssignment;
                    endAssignment->incomingNodes++;
                    i++;
                } else if (
                    conditionNodes[i]->targetNode == endAssignment && conditionNodes[i - 1]->hasAlternateTarget) {
                ...
                conditionNodes[i]->incomingNodes == 1 && conditionNodes[i - 1]->targetNode == conditionNodes[i]) {
        ...
        if (leftNode->targetNode->type < Node::END_TARGET) {
            if (rightNode->targetNode->type < Node::END_TARGET) {
                auto calculate_score = [](const auto& self, Node* const& node) -> int32_t {
                    ...
                    return Node::TYPE_PREFERENCE[node->type][!node->inverted] -
                           Node::TYPE_PREFERENCE[node->type][node->inverted];
                };
    ...
    static void invert_node(Node* const& node) {
        ...
        if (Node::TYPE_PREFERENCE[node->type][node->inverted])
            return;
        invert_node(node->leftNode);
        invert_node(node->rightNode);
        node->type = node->inverted ? (node->type == Node::AND ? Node::NOT_OR : Node::NOT_AND)
                                    : (node->type == Node::NOT_AND ? Node::OR : Node::AND);
    }
    ...
    void merge_nodes(Node* const& node, Node* const& targetNode) {
        ...
        if (!targetNode->hasAlternateTarget)
            node->hasAlternateTarget = false;
    ...
    Expression* build_expression(Node* const& node) {
        switch (node->type) {
            ...
            case Node::EQUAL:
                return build_binary(
                    node->inverted ? Node::NOT_EQUAL : Node::EQUAL, (*node->expressions)[0], (*node->expressions)[1]
                );
            case Node::NOT_EQUAL:
                return build_binary(
                    node->inverted ? Node::EQUAL : Node::NOT_EQUAL, (*node->expressions)[0], (*node->expressions)[1]
                );
            case Node::TRUTHY_TEST:
            ...
            case Node::BOOL_TRUTHY_TEST:
                return node->inverted ? build_not((*node->expressions).back())
                                      : build_not(build_not((*node->expressions).back()));
            case Node::BOOL_FALSY_TEST:
                return node->inverted ? build_not(build_not((*node->expressions).back()))
            ...
            case Node::OR:
                return node->inverted
                           ? build_not(build_binary(
                                 node->type, build_expression(node->leftNode), build_expression(node->rightNode)
                             ))
                           : build_binary(
                                 node->type, build_expression(node->leftNode), build_expression(node->rightNode)
                             );
    ...
    Expression* build_condition() {
        if (link_nodes()) {
            ...
            if (build_boolean_logic())
                return build_expression(conditionNodes.back());
        }
