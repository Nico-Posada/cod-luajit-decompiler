...
void Ast::build_functions(Function& function, uint32_t& functionCounter) {
    ...
    if (!function.hasDebugInfo)
        function.slotScopeCollector.build_upvalue_scopes();
    build_slot_scopes(function, function.block, nullptr);
    assert(
        function.slotScopeCollector.assert_scopes_closed(), "Failed to close slot scopes", bytecode.filePath, DEBUG_INFO
    );
    ...
    for (uint32_t i = function.block.size(); i--;) {
        ...
        switch (function.block[i]->instruction.type) {
            case Bytecode::BC_OP_FNEW:
                function.block[i]->function = new_function(
                    *function.get_constant(function.block[i]->instruction.d).prototype, function.level + 1
                );
                ...
                for (uint8_t j = function.block[i]->function->upvalues.size(); j--;) {
                    ...
                    if (!(function.block[i]->function->prototype.upvalues[j] & Bytecode::BC_UV_LOCAL)) {
                        function.block[i]->function->upvalues[j].slotScope =
                            function.upvalues[function.block[i]->function->upvalues[j].slot].slotScope;
                        continue;
                    }

                    function.block[i]->function->upvalues[j].local = true;
                    if (function.block[i]->function->upvalues[j].slot >= function.prototype.header.parameters)
            ...
            case Bytecode::BC_OP_JMP:
                function.block[i]->instruction.target =
                    function.block[i]->instruction.id +
                    (function.block[i]->instruction.d - Bytecode::BC_OP_JMP_BIAS + 1);
                continue;
        }
    }
...
void Ast::assign_debug_info(Function& function) {
    ...
    for (uint32_t i = function.parameterNames.size(); i < function.prototype.variableInfos.size(); i++) {
        assert(
            !activeLocalScopes.size() || function.prototype.variableInfos[i].scopeBegin >= activeLocalScopes.back() ||
        ...
        if (function.locals.size() &&
            ...
            switch (function.block[index]->instruction.type) {
                case Bytecode::BC_OP_KPRI:
                    if (function.block[index]->instruction.d)
                        break;
                case Bytecode::BC_OP_KNIL:
                    if ((function.block[index]->instruction.type == Bytecode::BC_OP_KPRI
                             ? function.block[index]->instruction.a
                             : function.block[index]->instruction.d) < activeLocalScopes.size()) {
                        while (activeLocalScopes.size() != function.locals.back().baseSlot) {
                            assert(
                                activeLocalScopes.size() &&
                                    activeLocalScopes.back() == function.prototype.variableInfos[i].scopeEnd,
                        ...
                        function.locals.back().scopeEnd = function.prototype.variableInfos[i].scopeEnd;
                        function.locals.back().excludeBlock =
                            function.locals[function.locals.size() - 2].scopeBegin ==
                                    function.locals[function.locals.size() - 2].scopeEnd
                                ? function.locals[function.locals.size() - 2].excludeBlock
                                : true;
...
void Ast::group_jumps(Function& function) {
    for (uint32_t i = function.block.size(); i--;) {
        switch (function.block[i]->instruction.type) {
            ...
            case Bytecode::BC_OP_ISF:
                ...
                function.slotScopeCollector.add_jump(
                    function.block[i]->instruction.id + 1, function.block[i]->instruction.target
                );
                continue;
            case Bytecode::BC_OP_UCLO:
                function.slotScopeCollector.add_upvalue_close(
                    function.block[i]->instruction.id,
                    function.block[i]->instruction.target,
                    function.block[i]->instruction.a
                );
            case Bytecode::BC_OP_JMP:
        ...
        switch (function.block[i]->instruction.type) {
            case Bytecode::BC_OP_UCLO:
                if (function.block[i]->instruction.d == Bytecode::BC_OP_JMP_BIAS ||
                    function.block[i]->instruction.target == get_extended_id_from_statement(function.block[i + 1])) {
            ...
            case Bytecode::BC_OP_ITERC:
                index = get_block_index_from_id(
                    function.block, function.labels[function.block[i]->instruction.label].jumpIds.front()
                );
                ...
                continue;
            case Bytecode::BC_OP_JMP:
                if (function.block[i]->type != AST_STATEMENT_GOTO)
                    continue;
                function.slotScopeCollector.add_jump(
                    function.block[i]->instruction.id, function.block[i]->instruction.target
                );
                if (function.block[i]->instruction.target == function.block[i]->instruction.id || !i ||
    ...
    for (uint32_t i = function.block.size(); i--;) {
        if (i && function.block[i]->type == AST_STATEMENT_RETURN &&
            function.block[i - 1]->type == AST_STATEMENT_RETURN &&
            function.is_valid_label(function.block[i]->instruction.label) &&
            function.labels[function.block[i]->instruction.label].jumpIds.size() == 1) {
            index = get_block_index_from_id(
                function.block, function.labels[function.block[i]->instruction.label].jumpIds.back()
            );

            if (index != INVALID_ID && function.block[index]->type == AST_STATEMENT_GOTO &&
        ...
        if (function.block[i]->instruction.type == Bytecode::BC_OP_RET0)
            function.block[i]->type = AST_STATEMENT_EMPTY;
        break;
    }
...
void Ast::build_loops(Function& function) {
    static const auto build_break_statements = [](std::vector<Statement*>& block, const uint32_t& breakTarget) -> void {
        for (uint32_t i = block.size(); i--;) {
            if (block[i]->type != AST_STATEMENT_GOTO || block[i]->instruction.target != breakTarget)
                continue;
    ...
    for (uint32_t i = function.block.size(); i--;) {
        if (function.block[i]->type != AST_STATEMENT_INSTRUCTION)
            continue;
        ...
        switch (function.block[i]->instruction.type) {
            ...
            case Bytecode::BC_OP_JMP:
                ...
                function.block[i]->block.insert(
                    function.block[i]->block.begin(),
                    function.block.begin() + i + 1,
                    function.block.begin() + targetIndex + 1
                );
                function.block.erase(function.block.begin() + i + 1, function.block.begin() + targetIndex + 2);
                function.slotScopeCollector.add_loop(
                    function.block[i]->instruction.id, function.block[i]->instruction.target
                );
            ...
            case Bytecode::BC_OP_FORI:
                ...
                function.block[i]->block.insert(
                    function.block[i]->block.begin(),
                    function.block.begin() + i + 1,
                    function.block.begin() + targetIndex
                );
            ...
            case Bytecode::BC_OP_LOOP:
                assert(
                    function.block[i]->instruction.target >= function.block[i]->instruction.id,
                ...
                if (function.block[i]->instruction.target == function.block[i]->instruction.id) {
                    assert(
                        i + 1 < function.block.size() && function.block[i + 1]->type == AST_STATEMENT_GOTO &&
                ...
                if (function.block[i]->block.size()
                    ...
                    for (uint32_t j = function.labels[function.block[i]->instruction.label].jumpIds.size();
                         j-- && function.labels[function.block[i]->instruction.label].jumpIds[j] >
                                    function.block[i]->instruction.id;) {
                        if (function.labels[function.block[i]->instruction.label].jumpIds[j] >=
                            function.block[i]->instruction.target)
                            continue;
                        targetIndex = get_block_index_from_id(
                            function.block[i]->block,
                            function.labels[function.block[i]->instruction.label].jumpIds[j] - 1
                        );

                        if (targetIndex != INVALID_ID &&
                            function.block[i]->block[targetIndex]->type == AST_STATEMENT_CONDITION) {
...
void Ast::build_local_scopes(Function& function, std::vector<Statement*>& block) {
    ...
    for (uint32_t i = function.locals.size(); i--;) {
        ...
        if (function.locals[i].scopeEnd > function.locals[i].scopeBegin) {
            ...
            if (scopeEndIndex == INVALID_ID)
                scopeEndIndex = block.size();

            while (block[scopeEndIndex - 1]->type == AST_STATEMENT_DECLARATION &&
                   block[scopeEndIndex - 1]->locals->excludeBlock) {
                scopeEndIndex--;
            }

            block[scopeBeginIndex]->block.reserve(scopeEndIndex - 1 - scopeBeginIndex);
            block[scopeBeginIndex]->block.insert(
                block[scopeBeginIndex]->block.begin(),
                block.begin() + scopeBeginIndex + 1,
                block.begin() + scopeEndIndex
            );
    ...
    for (uint32_t i = block.size(); i--;) {
        switch (block[i]->type) {
            case AST_STATEMENT_INSTRUCTION:
                ...
                switch (block[i]->instruction.type) {
                    ...
                    case Bytecode::BC_OP_LEN:
                        ...
                        block[i]->assignment.expressions.back()->unaryOperation->operand =
                            new_slot(block[i]->instruction.d);
                        block[i]->assignment.register_slots(
                            block[i]->assignment.expressions.back()->unaryOperation->operand
                        );
                        break;
                    ...
                    case Bytecode::BC_OP_CAT:
                        ...
                        switch (block[i]->instruction.type) {
                            ...
                            case Bytecode::BC_OP_MULNV:
                            case Bytecode::BC_OP_MULVV:
                                block[i]->assignment.expressions.back()->binaryOperation->type =
                        ...
                        switch (block[i]->instruction.type) {
                            ...
                            case Bytecode::BC_OP_DIVVN:
                            case Bytecode::BC_OP_MODVN:
                                block[i]->assignment.expressions.back()->binaryOperation->leftOperand =
                                    new_slot(block[i]->instruction.b);
                                block[i]->assignment.register_slots(
                                    block[i]->assignment.expressions.back()->binaryOperation->leftOperand
                                );
                                block[i]->assignment.expressions.back()->binaryOperation->rightOperand =
                                    new_number(function, block[i]->instruction.c);
                                break;
                            case Bytecode::BC_OP_ADDNV:
                            case Bytecode::BC_OP_SUBNV:
                            case Bytecode::BC_OP_MULNV:
                            ...
                            case Bytecode::BC_OP_POW:
                                block[i]->assignment.expressions.back()->binaryOperation->leftOperand =
                                    new_slot(block[i]->instruction.b);
                                block[i]->assignment.expressions.back()->binaryOperation->rightOperand =
                                    new_slot(block[i]->instruction.c);
                                block[i]->assignment.register_slots(
                                    block[i]->assignment.expressions.back()->binaryOperation->leftOperand,
                                    block[i]->assignment.expressions.back()->binaryOperation->rightOperand
                                );
                            ...
                            case Bytecode::BC_OP_CAT:
                                ...
                                for (Expression* expression = block[i]->assignment.expressions.back(); true;
                                     expression = expression->binaryOperation->rightOperand) {
                                    block[i]->assignment.register_slots(expression->binaryOperation->leftOperand);

                                    if (expression->binaryOperation->leftOperand->variable->slot ==
                                        block[i]->instruction.c - 1) {
                                    ...
                                    expression->binaryOperation->rightOperand =
                                        new_expression(AST_EXPRESSION_BINARY_OPERATION);
                                    expression->binaryOperation->rightOperand->binaryOperation->type =
                                        AST_BINARY_CONCATENATION;
                                    expression->binaryOperation->rightOperand->binaryOperation->leftOperand =
                                        new_slot(expression->binaryOperation->leftOperand->variable->slot + 1);
                                }
                    ...
                    case Bytecode::BC_OP_KNIL:
                        ...
                        if (block[i]->instruction.a == block[i]->instruction.d)
                            break;
                    ...
                    case Bytecode::BC_OP_UGET:
                        ...
                        block[i]->assignment.expressions.back()->variable->slotScope =
                            function.upvalues[block[i]->instruction.d].slotScope;
                        break;
                    ...
                    case Bytecode::BC_OP_USETP:
                        ...
                        block[i]->assignment.variables.back().slotScope =
                            function.upvalues[block[i]->instruction.a].slotScope;
                    ...
                    case Bytecode::BC_OP_GGET:
                        ...
                        block[i]->assignment.expressions.back()->variable->name =
                            function.get_constant(block[i]->instruction.d).string;
                        if (function.hasDebugInfo)
                            function.usedGlobals.emplace_back(&function.get_constant(block[i]->instruction.d).string);
                        break;
                    case Bytecode::BC_OP_HGGET:
                        block[i]->assignment.expressions.back() = new_expression(AST_EXPRESSION_VARIABLE);
                        block[i]->assignment.expressions.back()->variable->type = AST_VARIABLE_TABLE_INDEX;
                        block[i]->assignment.expressions.back()->variable->table =
                            new_expression(AST_EXPRESSION_VARIABLE);
                        ...
                        block[i]->assignment.expressions.back()->variable->tableIndex =
                            new_hash(function, block[i]->instruction.d);
                        break;
                    case Bytecode::BC_OP_GSET:
                        block[i]->assignment.variables.resize(1);
                        block[i]->assignment.variables.back().type = AST_VARIABLE_GLOBAL;
                        block[i]->assignment.variables.back().name =
                            function.get_constant(block[i]->instruction.d).string;
                        if (function.hasDebugInfo)
                            function.usedGlobals.emplace_back(&function.get_constant(block[i]->instruction.d).string);
                    ...
                    case Bytecode::BC_OP_TGETB:
                        ...
                        switch (block[i]->instruction.type) {
                            ...
                            case Bytecode::BC_OP_TGETS:
                                block[i]->assignment.expressions.back()->variable->tableIndex =
                                    new_string(function, block[i]->instruction.c);
                                check_valid_name(
                                    block[i]->assignment.expressions.back()->variable->tableIndex->constant
                                );
                                break;
                            case Bytecode::BC_OP_TGETB:
                                block[i]->assignment.expressions.back()->variable->tableIndex =
                                    new_literal(block[i]->instruction.c);
                                break;
                        }
                    ...
                    case Bytecode::BC_OP_TSETB:
                        ...
                        switch (block[i]->instruction.type) {
                            case Bytecode::BC_OP_TSETV:
                                ...
                                break;
                            case Bytecode::BC_OP_TSETS:
                                block[i]->assignment.variables.back().tableIndex =
                                    new_string(function, block[i]->instruction.c);
                    ...
                    case Bytecode::BC_OP_TSETM:
                        ...
                        assert(
                            function.get_number_constant(block[i]->instruction.d).type == Bytecode::BC_KNUM_NUM &&
                                (uint32_t)function.get_number_constant(block[i]->instruction.d).number,
                            "Multres table index is not a valid number constant",
                            bytecode.filePath,
                            DEBUG_INFO
                        );
                        block[i]->assignment.variables.back().multresIndex =
                            function.get_number_constant(block[i]->instruction.d).number;
                        block[i]->assignment.expressions.back() = new_slot(block[i]->instruction.a);
                    ...
                    case Bytecode::BC_OP_CALL:
                        ...
                        if (block[i]->instruction.b) {
                            if (block[i]->instruction.b == 1) {
                                ...
                                block[i]->assignment.expressions.back()->functionCall->returnCount =
                        ...
                        block[i]->assignment.expressions.back()->functionCall->function =
                            new_slot(block[i]->instruction.a);
                        block[i]->assignment.register_slots(
                            block[i]->assignment.expressions.back()->functionCall->function
                        );
                        block[i]->assignment.expressions.back()->functionCall->arguments.resize(
                            block[i]->instruction.c + (block[i]->instruction.type == Bytecode::BC_OP_CALLM ? 0 : -1),
                            nullptr
                        );
                        if (block[i]->assignment.expressions.back()->functionCall->arguments.size())
                            block[i]->assignment.isPotentialMethod = true;

                        for (uint8_t j = 0; j < block[i]->assignment.expressions.back()->functionCall->arguments.size();
                             j++) {
                            block[i]->assignment.expressions.back()->functionCall->arguments[j] =
                                new_slot(block[i]->instruction.a + (isFR2Enabled ? 2 : 1) + j);
                            block[i]->assignment.register_slots(
                                block[i]->assignment.expressions.back()->functionCall->arguments[j]
                            );
                        }
                        ...
                        if (block[i]->instruction.type == Bytecode::BC_OP_CALLM) {
                            block[i]->assignment.expressions.back()->functionCall->multresArgument =
                                new_slot(block[i]->instruction.a + (isFR2Enabled ? 2 : 1) + block[i]->instruction.c);
                            block[i]
                                ->assignment.expressions.back()
                                ->functionCall->multresArgument->variable->isMultres = true;
                            block[i]->assignment.register_slots(
                                block[i]->assignment.expressions.back()->functionCall->multresArgument
                            );
                        }
                    ...
                    case Bytecode::BC_OP_VARG:
                        ...
                        if (block[i]->instruction.b) {
                            if (block[i]->instruction.b == 1) {
                                ...
                                block[i]->assignment.expressions.back()->returnCount =
                                    block[i]->assignment.variables.size();
                            }
                        } else {
                            block[i]->assignment.variables.resize(1);
            ...
            case AST_STATEMENT_RETURN:
                if (i && block[i - 1]->type == AST_STATEMENT_EMPTY &&
                ...
                switch (block[i]->instruction.type) {
                    ...
                    case Bytecode::BC_OP_CALLT:
                        ...
                        block[i]->assignment.multresReturn->functionCall->arguments.resize(
                            block[i]->instruction.d + (block[i]->instruction.type == Bytecode::BC_OP_CALLMT ? 0 : -1),
                            nullptr
                        );
                        if (block[i]->assignment.multresReturn->functionCall->arguments.size())
                            block[i]->assignment.isPotentialMethod = true;

                        for (uint8_t j = 0; j < block[i]->assignment.multresReturn->functionCall->arguments.size();
                             j++) {
                            block[i]->assignment.multresReturn->functionCall->arguments[j] =
                                new_slot(block[i]->instruction.a + (isFR2Enabled ? 2 : 1) + j);
                            block[i]->assignment.register_slots(
                                block[i]->assignment.multresReturn->functionCall->arguments[j]
                            );
                        ...
                        if (block[i]->instruction.type == Bytecode::BC_OP_CALLMT) {
                            block[i]->assignment.multresReturn->functionCall->multresArgument =
                                new_slot(block[i]->instruction.a + (isFR2Enabled ? 2 : 1) + block[i]->instruction.d);
                            block[i]->assignment.multresReturn->functionCall->multresArgument->variable->isMultres =
                                true;
                            block[i]->assignment.register_slots(
                                block[i]->assignment.multresReturn->functionCall->multresArgument
                            );
                    ...
                    case Bytecode::BC_OP_RET:
                    case Bytecode::BC_OP_RET1:
                        block[i]->assignment.expressions.resize(
                            block[i]->instruction.type == Bytecode::BC_OP_RET1
                                ? 1
                                : block[i]->instruction.d +
                                      (block[i]->instruction.type == Bytecode::BC_OP_RETM ? 0 : -1),
                            nullptr
                        );
                        ...
                        if (block[i]->instruction.type == Bytecode::BC_OP_RETM) {
                            block[i]->assignment.multresReturn =
                                new_slot(block[i]->instruction.a + block[i]->instruction.d);
            ...
            case AST_STATEMENT_NUMERIC_FOR:
                ...
                assert(
                    !function.hasDebugInfo ||
                        (block[i]->locals && block[i]->assignment.variables.back().slot == block[i]->locals->baseSlot &&
                         block[i]->locals->names.size() == 1),
                    "Numeric for loop variable does not match with debug info",
                    bytecode.filePath,
                    DEBUG_INFO
                );
                ...
                block[i]->assignment.register_slots(
                    block[i]->assignment.expressions[0],
                    block[i]->assignment.expressions[1],
                    block[i]->assignment.expressions[2]
                );
                continue;
            case AST_STATEMENT_GENERIC_FOR:
                block[i]->assignment.variables.resize(block[i]->instruction.b - 1);
                ...
                assert(
                    ...
                    "Generic for loop variables do not match with debug info",
                    bytecode.filePath,
                    DEBUG_INFO
                );
                block[i]->assignment.expressions.resize(3, nullptr);
...
void Ast::build_slot_scopes(Function& function, std::vector<Statement*>& block, BlockInfo* const& previousBlock) {
    ...
    for (uint32_t i = block.size(); i--;) {
        switch (block[i]->type) {
            ...
            case AST_STATEMENT_GENERIC_FOR:
                for (uint32_t j = block[i]->assignment.variables.size(); j--;) {
                    assert(
                        !function.slotScopeCollector.slotInfos[block[i]->assignment.variables[j].slot].activeSlotScope,
                        "Slot scope does not match with for loop variable",
                        bytecode.filePath,
                        DEBUG_INFO
                    );
                    function.slotScopeCollector.begin_scope(
                        block[i]->assignment.variables[j].slot, block[i]->instruction.target - 1
                    );
                }
            case AST_STATEMENT_LOOP:
                ...
                blockInfo.index = i;
                build_slot_scopes(
                    function, block[i]->block, block[i]->type == AST_STATEMENT_LOOP ? &blockInfo : nullptr
                );
            ...
            case AST_STATEMENT_DECLARATION:
                ...
                for (uint8_t j = function.slotScopeCollector.slotInfos.size();
                     j-- && j >= block[i]->locals->baseSlot;) {
                    if (!function.slotScopeCollector.slotInfos[j].activeSlotScope)
                        continue;

                    for (uint8_t k = j; true; k--) {
                        assert(
                            function.slotScopeCollector.slotInfos[k].activeSlotScope &&
                        ...
                        function.slotScopeCollector.close_scope(
                            k, block[i + 1]->assignment.variables.back().slotScope, block[i]->locals->scopeEnd
                        );
                        if (k == block[i]->locals->baseSlot)
                            break;
                    }
                ...
                for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
                    function.slotScopeCollector.begin_scope(
                        block[i]->assignment.variables[j].slot, block[i]->locals->scopeEnd
                    );
                }
                ...
                for (uint8_t j = function.slotScopeCollector.slotInfos.size();
                     j-- && j >= block[i]->assignment.variables.back().slot + 1;) {
                    if (!function.slotScopeCollector.slotInfos[j].activeSlotScope)
                        continue;
                    ...
                    for (uint8_t k = j; true; k--) {
                        ...
                        function.slotScopeCollector.close_scope(
                            k,
                            block[i]->block.front()->assignment.variables.back().slotScope,
                            block[i]->locals->scopeBegin
                        );
                        if (k == block[i]->assignment.variables.back().slot + 1)
                            break;
                    }
        ...
        if (block[i]->instruction.id != INVALID_ID) {
            ...
            if (function.is_valid_label(targetLabel) && function.labels[targetLabel].jumpIds.front() < id &&
                (extendedTargetLabel == targetLabel || function.labels[extendedTargetLabel].target > id ||
                                    ...
                                    case AST_STATEMENT_CONDITION:
                                        if (block[index]->assignment.expressions.size() == 1) {
                                            if (block[index]->assignment.variables.size()) {
                                                if (function.slotScopeCollector
                                                        .slotInfos[block[index]->assignment.variables.back().slot]
                                                        .activeSlotScope &&
                                                    function.slotScopeCollector
                                                            .slotInfos[block[index]->assignment.variables.back().slot]
                                                            .minScopeBegin == block[index]->instruction.id) {
                                                ...
                                                function.slotScopeCollector
                                                    .slotInfos
                                                        [block[index]->assignment.expressions.back()->variable->slot]
                                                    .activeSlotScope &&
                                                function.slotScopeCollector
                                                        .slotInfos[block[index]
                                                                       ->assignment.expressions.back()
                                                                       ->variable->slot]
                                                        .minScopeBegin == block[index]->instruction.id) {
                                                isPossibleCondition = true;
                            ...
                            if (block[i]->assignment.variables.size() == 1) {
                                switch (block[i]->assignment.variables.back().type) {
                                    case AST_VARIABLE_SLOT:
                                        if (function.slotScopeCollector
                                                .slotInfos[block[i]->assignment.variables.back().slot]
                                                .activeSlotScope &&
                                            function.slotScopeCollector
                                                    .slotInfos[block[i]->assignment.variables.back().slot]
                                                    .minScopeBegin == block[index]->instruction.id) {
                                            ...
                                            if (i >= 5 && index <= i - 4 &&
                                                (((block[i - 3]->type == AST_STATEMENT_GOTO ||
                                                  ...
                                                  block[i - 3]->instruction.target ==
                                                      function.labels[extendedTargetLabel].target) ||
                                                ...
                                                block[i]->assignment.expressions.back()->constant->type ==
                                                    AST_CONSTANT_TRUE &&
                                                (block[i - 1]->type == AST_STATEMENT_GOTO ||
                                                 block[i - 1]->type == AST_STATEMENT_BREAK) &&
                                                !function.is_valid_label(block[i - 1]->instruction.label) &&
                                                block[i - 1]->instruction.target ==
                                                    function.labels[targetLabel].target &&
                                                block[i - 2]->type == AST_STATEMENT_ASSIGNMENT &&
                                                block[i - 2]->assignment.expressions.back()->type ==
                                                    AST_EXPRESSION_CONSTANT &&
                                                block[i - 2]->assignment.expressions.back()->constant->type ==
                                                    AST_CONSTANT_FALSE &&
                                                (function.is_valid_label(block[i]->instruction.label) ||
                                    ...
                                    case AST_VARIABLE_TABLE_INDEX:
                                        if (function.slotScopeCollector
                                                .slotInfos[block[i]->assignment.variables.back().table->variable->slot]
                                                .activeSlotScope &&
                                            function.slotScopeCollector
                                                    .slotInfos
                                                        [block[i]->assignment.variables.back().table->variable->slot]
                                                    .minScopeBegin == block[index]->instruction.id) {
                                            isPossibleCondition = true;
                    ...
                    if (isPossibleCondition) {
                        ...
                        if (!hasBoolConstruct || index == i - 4) {
                            ...
                            if (block[index]->type == AST_STATEMENT_ASSIGNMENT &&
                                ...
                                block[index]->assignment.variables.back().type == AST_VARIABLE_SLOT) {
                                if (block[index]->assignment.variables.back().slot == targetSlot)
                                    isPossibleCondition = true;
                            } else if (
                                (block[index]->type == AST_STATEMENT_ASSIGNMENT &&
                                 block[index]->assignment.variables.size() == 1 &&
                                ...
                                while (index--) {
                                    switch (block[index]->type) {
                                        case AST_STATEMENT_CONDITION:
                                            if (!block[index]->assignment.variables.size() &&
                                                block[index]->instruction.target ==
                                                    function.labels[extendedTargetLabel].target)
                                                continue;
                                        case AST_STATEMENT_GOTO:
                                        case AST_STATEMENT_BREAK:
                                            if (block[index]->instruction.target ==
                                                    function.labels[targetLabel].target ||
                                                block[index]->instruction.target ==
                                                    function.labels[extendedTargetLabel].target ||
                                                block[index]->instruction.target >
                                        ...
                                        case AST_STATEMENT_ASSIGNMENT:
                                            if (block[index]->assignment.variables.size() == 1 &&
                                                ...
                                                if (block[index]->assignment.isTableConstructor &&
                                                    (hasBoolConstruct ||
                                                     block[index]->instruction.id >
                                                         function.labels[targetLabel].jumpIds.front()) &&
                                                    function.is_valid_block_range(
                                                        block[index]->instruction.id,
                                                        block[hasBoolConstruct ? i - 4 : i]->instruction.id,
                                                        true
                                                    ))
                                ...
                                switch (block[index]->type) {
                                    case AST_STATEMENT_CONDITION:
                                        ...
                                        if (block[index]->assignment.variables.size()) {
                                            ...
                                            index &&
                                            block[index]->assignment.expressions.back()->variable->slot == targetSlot) {
                                            index--;
                                            ...
                                            if (block[index]->type == AST_STATEMENT_ASSIGNMENT &&
                                                ...
                                                (block[index]->type == AST_STATEMENT_ASSIGNMENT &&
                                                 block[index]->assignment.variables.size() == 1 &&
                                                 block[index]->assignment.variables.back().type ==
                                                     AST_VARIABLE_TABLE_INDEX &&
                                                 block[index]->assignment.variables.back().table->variable->slot ==
                                                     targetSlot &&
                                                 !function.is_valid_label(block[index + 1]->instruction.label)) ||
                                                (block[index]->type == AST_STATEMENT_CONDITION &&
                                                 block[index]->instruction.target ==
                                                    ...
                                                    switch (block[index]->type) {
                                                        ...
                                                        case AST_STATEMENT_ASSIGNMENT:
                                                            if (block[index]->assignment.variables.size() == 1 &&
                                                                block[index]->assignment.variables.back().type ==
                                                                    AST_VARIABLE_SLOT &&
                                                                block[index]->assignment.variables.back().slot ==
                                                                    targetSlot) {
                                                                if (block[index]->assignment.isTableConstructor &&
                                                                    function.is_valid_block_range(
                                                                        block[index]->instruction.id,
                                    ...
                                    case AST_STATEMENT_GOTO:
                                    case AST_STATEMENT_BREAK:
                                        if (index-- && block[index]->assignment.variables.size() == 1 &&
                                            block[index]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                                            block[index]->assignment.variables.back().slot == targetSlot &&
                                            get_constant_type(block[index]->assignment.expressions.back()))
                                            isPossibleCondition = true;
                                                ...
                                                case AST_VARIABLE_SLOT:
                                                    if (block[j]->function) {
                                                        for (uint8_t k = block[j]->function->upvalues.size(); k--;) {
                                                            if (!block[j]->function->upvalues[k].local ||
                                                                block[j]->function->upvalues[k].slot != targetSlot)
                                                                continue;
                                                            isPossibleCondition = false;
                                                            break;
                                                        }
                            ...
                            if (isPossibleCondition) {
                                for (uint32_t j = conditionBlocks.size(); isPossibleCondition && j--;) {
                                    for (uint32_t k = conditionBlocks[j].size(); k--;) {
                                        if (!function.is_valid_label(conditionBlocks[j][k]->instruction.label) ||
                                            function.labels[conditionBlocks[j][k]->instruction.label].jumpIds.back() <
                                                conditionBlocks[j][k]->instruction.id)
                                            continue;
                                        isPossibleCondition = false;
                                        break;
                                    }
                                ...
                                if (isPossibleCondition) {
                                    ...
                                    for (uint32_t j = 0; j < conditionBlocks.size(); j++) {
                                        if (j && (!hasBoolConstruct || j != 2 ||
                                                  conditionBlocks[j].back()->type != AST_STATEMENT_CONDITION)) {
                                            (*targetSlotScope)->usages++;
                                            function.slotScopeCollector.slotInfos[targetSlot].activeSlotScope =
                                                targetSlotScope;
                                        }

                                        build_slot_scopes(function, conditionBlocks[j], nullptr);
                                        i -= conditionBlocks[j].size();
                                        if (!function.slotScopeCollector.slotInfos[targetSlot].activeSlotScope ||
                                            j == conditionBlocks.size() - 1)
                                            continue;

                                        while (function.slotScopeCollector.slotInfos[targetSlot].slotScopes.back() !=
                                               targetSlotScope) {
                                            (*targetSlotScope)->usages +=
                                                (*function.slotScopeCollector.slotInfos[targetSlot].slotScopes.back())
                                                    ->usages +
                                                1;

                                            for (uint32_t k = (*function.slotScopeCollector.slotInfos[targetSlot]
                                                                    .slotScopes.back())
                                                                  ->mergedScopes.size();
                                                 k--;) {
                                                *(*function.slotScopeCollector.slotInfos[targetSlot].slotScopes.back())
                                                     ->mergedScopes[k] = *targetSlotScope;
                                            }

                                            (*targetSlotScope)
                                                ->mergedScopes.insert(
                                                    (*targetSlotScope)->mergedScopes.begin(),
                                                    (*function.slotScopeCollector.slotInfos[targetSlot]
                                                          .slotScopes.back())
                                                        ->mergedScopes.begin(),
                                                    (*function.slotScopeCollector.slotInfos[targetSlot]
                                                          .slotScopes.back())
                                            ...
                                            (*targetSlotScope)
                                                ->mergedScopes.emplace_back(&(*function.slotScopeCollector
                                                                                   .slotInfos[targetSlot]
                                                                                   .slotScopes.back())
                                                                                 ->slotScope);
                                            *function.slotScopeCollector.slotInfos[targetSlot].slotScopes.back() =
                                                *targetSlotScope;
                                        ...
                                        function.slotScopeCollector.slotInfos[targetSlot].activeSlotScope =
                                            targetSlotScope;
                                        function.slotScopeCollector.extend_scope(
                                            targetSlot,
                                            function.get_scope_begin_from_label(
                                                targetLabel, (*targetSlotScope)->scopeEnd
                                            )
                                        );
                                        break;
                                    }
        ...
        if (block[i]->function) {
            for (uint8_t j = block[i]->function->upvalues.size(); j--;) {
                if (!block[i]->function->upvalues[j].local)
                    continue;
                if (block[i]->function->upvalues[j].slot == block[i]->assignment.variables.back().slot)
                    block[i]->function->assignmentSlotIsUpvalue = true;
                function.slotScopeCollector.add_to_scope(
                    block[i]->function->upvalues[j].slot, block[i]->function->upvalues[j].slotScope, id
                );
            }
        }
            ...
            switch (block[i]->assignment.variables[j].type) {
                case AST_VARIABLE_SLOT:
                    if (block[i]->type != AST_STATEMENT_DECLARATION ||
                        function.slotScopeCollector.slotInfos[block[i]->assignment.variables[j].slot].minScopeBegin >=
                            id) {
                        function.slotScopeCollector.close_scope(
                            block[i]->assignment.variables[j].slot, block[i]->assignment.variables[j].slotScope, id
                        );
                        continue;
                    }
                ...
                case AST_VARIABLE_TABLE_INDEX:
                    function.slotScopeCollector.add_to_scope(
                        block[i]->assignment.variables[j].table->variable->slot,
                        block[i]->assignment.variables[j].table->variable->slotScope,
                        id
                    );
                    continue;
            }
        ...
        assert(
            !block[i]->assignment.variables.size() ||
                ...
                ((*block[i]->assignment.variables.front().slotScope)->usages == 1 &&
                  ...
                  *function.slotScopeCollector.slotInfos[block[i]->assignment.variables.front().slot].activeSlotScope !=
                      *block[i]->assignment.variables.front().slotScope)),
        ...
        for (uint8_t j = block[i]->assignment.openSlots.size(); j--;) {
            function.slotScopeCollector.add_to_scope(
                (*block[i]->assignment.openSlots[j])->variable->slot,
                (*block[i]->assignment.openSlots[j])->variable->slotScope,
                id
            );
        }
        ...
        if (block[i]->instruction.id != INVALID_ID) {
            ...
            if (function.is_valid_label(block[i]->instruction.label)) {
                ...
                if (id > block[i]->instruction.id)
                    function.slotScopeCollector.merge_scopes(id);
                function.slotScopeCollector.extend_scopes(
                    function.get_scope_begin_from_label(block[i]->instruction.label, id)
                );
            }
        }
    }
...
void Ast::eliminate_slots(Function& function, std::vector<Statement*>& block, BlockInfo* const& previousBlock) {
    static bool (*const has_self_reference)(const uint8_t&, Expression* const&) =
        [](const uint8_t& targetSlot, Expression* const& expression) -> bool {
        switch (expression->type) {
            case AST_EXPRESSION_FUNCTION:
                for (uint8_t i = expression->function->upvalues.size(); i--;) {
                    if (expression->function->upvalues[i].local && expression->function->upvalues[i].slot == targetSlot)
                        return true;
                }
            ...
            case AST_EXPRESSION_VARIABLE:
                switch (expression->variable->type) {
                    ...
                    case AST_VARIABLE_TABLE_INDEX:
                        return has_self_reference(targetSlot, expression->variable->table) ||
                               has_self_reference(targetSlot, expression->variable->tableIndex);
                }

                break;
            case AST_EXPRESSION_FUNCTION_CALL:
                if (has_self_reference(targetSlot, expression->functionCall->function))
                    return true;

                for (uint32_t i = expression->functionCall->arguments.size(); i--;) {
                    if (has_self_reference(targetSlot, expression->functionCall->arguments[i]))
                        return true;
                }

                if (expression->functionCall->multresArgument)
                    return has_self_reference(targetSlot, expression->functionCall->multresArgument);
                break;
            case AST_EXPRESSION_TABLE:
                for (uint32_t i = expression->table->fields.size(); i--;) {
                    if (has_self_reference(targetSlot, expression->table->fields[i].key) ||
                        has_self_reference(targetSlot, expression->table->fields[i].value))
                        return true;
                }

                if (expression->table->multresField)
                    return has_self_reference(targetSlot, expression->table->multresField);
                break;
            case AST_EXPRESSION_BINARY_OPERATION:
                return has_self_reference(targetSlot, expression->binaryOperation->leftOperand) ||
                       has_self_reference(targetSlot, expression->binaryOperation->rightOperand);
    ...
    for (uint32_t i = 0; i < block.size(); i++) {
        switch (block[i]->type) {
            case AST_STATEMENT_CONDITION:
                if (block[i]->condition.allowSlotSwap && i && !function.is_valid_label(block[i]->instruction.label) &&
                    block[i - 1]->type == AST_STATEMENT_ASSIGNMENT && block[i - 1]->assignment.variables.size() == 1 &&
                    block[i - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                    (*block[i - 1]->assignment.variables.back().slotScope)->usages == 1 &&
                    block[i - 1]->assignment.variables.back().slot ==
                        block[i]->assignment.expressions[0]->variable->slot) {
            ...
            case AST_STATEMENT_DECLARATION:
                while (i && !function.is_valid_label(block[i]->instruction.label)) {
                    switch (block[i - 1]->type) {
                        case AST_STATEMENT_ASSIGNMENT:
                            if (block[i - 1]->assignment.variables.front().slot <=
                                block[i]
                                    ->assignment.expressions[block[i]->assignment.openSlots.size() - 1]
                                    ->variable->slot)
                                break;
                            assert(
                        ...
                        case AST_STATEMENT_FUNCTION_CALL:
                            block[i]->assignment.expressions.emplace(
                                block[i]->assignment.expressions.begin() + block[i]->assignment.openSlots.size(),
                                block[i - 1]->assignment.expressions.back()
                            );
                            block[i]->instruction.label = block[i - 1]->instruction.label;
                            i--;
                            block.erase(block.begin() + i);
                    ...
                    if (block[i - 1]->type == AST_STATEMENT_ASSIGNMENT &&
                        block[i - 1]->assignment.variables.size() != 1) {
                        assert(
                            block[i]->assignment.expressions.size() == block[i]->assignment.openSlots.size() &&
                                block[i]->assignment.expressions.back()->variable->slot ==
                                    block[i - 1]->assignment.variables.back().slot,
                        ...
                        while (true) {
                            function.slotScopeCollector.remove_scope(
                                block[i]->assignment.expressions.back()->variable->slot,
                                block[i]->assignment.expressions.back()->variable->slotScope
                            );
                            block[i]->assignment.openSlots.pop_back();

                            if (block[i]->assignment.expressions.back()->variable->slot !=
                                block[i - 1]->assignment.variables.front().slot) {
                ...
                switch (block[i]->assignment.variables.back().type) {
                    case AST_VARIABLE_SLOT:
                        if (block[i]->assignment.expressions.back()->type == AST_EXPRESSION_BINARY_OPERATION &&
                            block[i]->assignment.expressions.back()->binaryOperation->type !=
                                AST_BINARY_CONCATENATION &&
                            block[i]->assignment.openSlots.size() == 2 && i >= 2 &&
                            !function.is_valid_label(block[i]->instruction.label) &&
                            !function.is_valid_label(block[i - 1]->instruction.label) &&
                            ...
                            (*block[i - 1]->assignment.variables.back().slotScope)->usages == 1 &&
                            block[i - 1]->assignment.variables.back().slot ==
                                block[i]->assignment.expressions.back()->binaryOperation->leftOperand->variable->slot &&
                            ...
                            block[i - 2]->assignment.variables.back().slot ==
                                block[i]
                                    ->assignment.expressions.back()
                                    ->binaryOperation->rightOperand->variable->slot) {
                            block[i]->assignment.openSlots[0] =
                                &block[i]->assignment.expressions.back()->binaryOperation->rightOperand;
                            block[i]->assignment.openSlots[1] =
                                &block[i]->assignment.expressions.back()->binaryOperation->leftOperand;
                        }
                    ...
                    case AST_VARIABLE_TABLE_INDEX:
                        if (!block[i]->assignment.variables.back().isMultres &&
                            ...
                            i >= 3 && !function.is_valid_label(block[i]->instruction.label) &&
                            !function.is_valid_label(block[i - 1]->instruction.label) &&
                            !function.is_valid_label(block[i - 2]->instruction.label) &&
                            block[i - 1]->type == AST_STATEMENT_ASSIGNMENT &&
                            block[i - 1]->assignment.variables.size() == 1 &&
                            ...
                            (*block[i - 1]->assignment.variables.back().slotScope)->usages == 1 &&
                            block[i - 1]->assignment.variables.back().slot ==
                                block[i]->assignment.variables.back().tableIndex->variable->slot &&
                            ...
                            block[i - 3]->assignment.variables.back().slot ==
                                block[i]->assignment.variables.back().table->variable->slot &&
        ...
        if (block[i]->type == AST_STATEMENT_DECLARATION && block[i]->assignment.openSlots.size() == 1 &&
            (*(*block[i]->assignment.openSlots.back())->variable->slotScope)->usages > 1 && i &&
            block[i - 1]->type == AST_STATEMENT_ASSIGNMENT && block[i - 1]->assignment.variables.size() == 1 &&
            block[i - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT && block[i - 1]->function &&
            block[i - 1]->function->assignmentSlotIsUpvalue &&
            block[i - 1]->assignment.variables.back().slot ==
                (*block[i]->assignment.openSlots.back())->variable->slot) {
            ...
            *block[i - 1]->assignment.variables.back().slotScope = *block[i]->assignment.variables.back().slotScope;
            ...
            i--;
            function.slotScopeCollector.remove_scope(
                block[i]->assignment.variables.back().slot, block[i]->assignment.variables.back().slotScope
            );
            block.erase(block.begin() + i);
        } else {
            for (uint8_t j = block[i]->assignment.openSlots.size();
                 j-- && i && !function.is_valid_label(block[i]->instruction.label) &&
                 block[i - 1]->type == AST_STATEMENT_ASSIGNMENT && block[i - 1]->assignment.variables.size() == 1 &&
                 block[i - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                 (*block[i - 1]->assignment.variables.back().slotScope)->usages == 1;) {
                if (j == 1 && block[i]->assignment.isPotentialMethod && i >= 2 &&
                    !function.is_valid_label(block[i - 1]->instruction.label) &&
                    block[i - 1]->assignment.variables.back().slot ==
                        (*block[i]->assignment.openSlots.front())->variable->slot &&
                    ...
                    block[i - 1]->assignment.expressions.back()->variable->tableIndex->type ==
                        AST_EXPRESSION_CONSTANT &&
                    block[i - 1]->assignment.expressions.back()->variable->tableIndex->constant->isName &&
                    block[i - 2]->type == AST_STATEMENT_ASSIGNMENT && block[i - 2]->assignment.variables.size() == 1 &&
                    block[i - 2]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                    (*block[i - 2]->assignment.variables.back().slotScope)->usages == 1 &&
                    block[i - 2]->assignment.variables.back().slot ==
                        (*block[i]->assignment.openSlots[j])->variable->slot &&
                    ...
                    block[i - 2]->assignment.expressions.back()->variable->slot ==
                        block[i - 1]->assignment.expressions.back()->variable->table->variable->slot) {
                    if (block[i]->type == AST_STATEMENT_RETURN) {
                        block[i]->assignment.multresReturn->functionCall->isMethod = true;
                        block[i]->assignment.multresReturn->functionCall->arguments.erase(
                            block[i]->assignment.multresReturn->functionCall->arguments.begin()
                        );
                    } else {
                        block[i]->assignment.expressions.back()->functionCall->isMethod = true;
                        block[i]->assignment.expressions.back()->functionCall->arguments.erase(
                            block[i]->assignment.expressions.back()->functionCall->arguments.begin()
                        );
                    }

                    block[i]->assignment.openSlots.erase(block[i]->assignment.openSlots.begin() + j);
                    block[i]->assignment.openSlots.emplace(
                        block[i]->assignment.openSlots.begin(),
                        &block[i - 1]->assignment.expressions.back()->variable->table
                    );
                    function.slotScopeCollector.remove_scope(
                        block[i - 2]->assignment.variables.back().slot,
                        block[i - 2]->assignment.variables.back().slotScope
                    );
                ...
                if (block[i - 1]->assignment.variables.back().slot !=
                    (*block[i]->assignment.openSlots[j])->variable->slot)
                    continue;
                assert(
                    block[i - 1]->assignment.variables.back().isMultres ==
                        (*block[i]->assignment.openSlots[j])->variable->isMultres,
                ...
                *block[i]->assignment.openSlots[j] = block[i - 1]->assignment.expressions.back();
                ...
                if (!j && block[i]->assignment.allowedConstantType != NUMBER_CONSTANT &&
                    get_constant_type(block[i]->assignment.expressions.back()) >
                        block[i]->assignment.allowedConstantType) {
                    *block[i]->assignment.openSlots[j] = expression;
                    break;
                }

                function.slotScopeCollector.remove_scope(
                    block[i - 1]->assignment.variables.back().slot, block[i - 1]->assignment.variables.back().slotScope
                );
                block[i]->instruction.label = block[i - 1]->instruction.label;
                i--;
                block.erase(block.begin() + i);
        ...
        assert(
            !block[i]->assignment.openSlots.size() ||
                    ...
                    switch (block[i]->assignment.variables.back().type) {
                        case AST_VARIABLE_SLOT:
                            ...
                            if ((*block[i]->assignment.variables.back().slotScope)->usages >= 2) {
                                if ((*block[i]->assignment.variables.back().slotScope)->scopeBegin >=
                                        function.labels[targetLabel].jumpIds.front() ||
                                    (extendedTargetLabel != targetLabel &&
                                     (function.labels[extendedTargetLabel].target <= block[i]->instruction.id ||
                                      function.labels[extendedTargetLabel].target >=
                                          function.labels[targetLabel].jumpIds.front())) ||
                                    has_self_reference(
                                        block[i]->assignment.variables.back().slot,
                                        block[i]->assignment.expressions.back()
                                    ))
                                    break;
                                index =
                                    get_block_index_from_id(block, function.labels[targetLabel].jumpIds.front() - 1);
                                if (index == INVALID_ID)
                                    break;
                                ...
                                switch (block[index]->type) {
                                    case AST_STATEMENT_CONDITION:
                                        if (block[index]->assignment.variables.size()) {
                                            if ((*block[index]->assignment.variables.back().slotScope)->scopeBegin ==
                                                    block[index]->instruction.id &&
                                                *block[index]->assignment.variables.back().slotScope ==
                                                    *block[i]->assignment.variables.back().slotScope)
                                                break;
                                        } else if (
                                            index && block[index]->assignment.expressions.size() == 1 &&
                                            ...
                                            (*block[index - 1]->assignment.variables.back().slotScope)->scopeBegin ==
                                                block[index - 1]->instruction.id &&
                                    ...
                                    case AST_STATEMENT_ASSIGNMENT:
                                        if (block[index]->assignment.variables.size() != 1 ||
                                            block[index]->assignment.variables.back().type != AST_VARIABLE_SLOT ||
                                            (*block[index]->assignment.variables.back().slotScope)->scopeBegin !=
                                                block[index]->instruction.id ||
                                            *block[index]->assignment.variables.back().slotScope !=
                                                *block[i]->assignment.variables.back().slotScope ||
                                            (index != i - 4 &&
                                             (block[index]->assignment.expressions.back()->type !=
                                                  AST_EXPRESSION_CONSTANT ||
                                              !get_constant_type(block[index]->assignment.expressions.back()))))
                                            index = INVALID_ID;
                                        break;
                                ...
                                if (i >= 3 && block[i]->type == AST_STATEMENT_ASSIGNMENT &&
                                    ...
                                    *block[i - 2]->assignment.variables.back().slotScope ==
                                    ...
                                    switch (block[i - 3]->type) {
                                        case AST_STATEMENT_CONDITION:
                                            if (block[i - 3]->assignment.expressions.size() == 2 &&
                                                block[i - 3]->instruction.target == block[i]->instruction.id)
                                                hasBoolConstruct = true;
                                            break;
                                        case AST_STATEMENT_GOTO:
                                        case AST_STATEMENT_BREAK:
                                            if (i < 5 || function.is_valid_label(block[i - 3]->instruction.label) ||
                                                ...
                                                block[i - 4]->assignment.variables.back().slot !=
                                                    block[i]->assignment.variables.back().slot)
                                                break;

                                            if (index == i - 2 &&
                                                !function.is_valid_label(block[i]->instruction.label)) {
                                                if (function.labels[block[i - 2]->instruction.label].jumpIds.front() >
                                                    block[i - 2]->instruction.id)
                                                    break;
                                                index = get_block_index_from_id(
                                                    block,
                                                    function.labels[block[i - 2]->instruction.label].jumpIds.front() - 1
                                                );
                                    ...
                                    if (hasBoolConstruct) {
                                        if ((function.is_valid_label(block[i]->instruction.label) &&
                                             function.labels[block[i]->instruction.label].jumpIds.back() >=
                                                 block[i]->instruction.id) ||
                                            (function.is_valid_label(block[i - 2]->instruction.label) &&
                                             function.labels[block[i - 2]->instruction.label].jumpIds.back() >=
                                                 block[i - 2]->instruction.id))
                                            break;

                                        if (function.is_valid_label(block[i]->instruction.label)) {
                                            for (uint32_t j =
                                                     function.labels[block[i]->instruction.label].jumpIds.size();
                                                 j--;) {
                                                targetIndex = get_block_index_from_id(
                                                    block, function.labels[block[i]->instruction.label].jumpIds[j] - 1
                                                );
                                        ...
                                        if (hasBoolConstruct &&
                                            function.is_valid_label(block[i - 2]->instruction.label)) {
                                            for (uint32_t j =
                                                     function.labels[block[i - 2]->instruction.label].jumpIds.size();
                                                 j--;) {
                                                targetIndex = get_block_index_from_id(
                                                    block,
                                                    function.labels[block[i - 2]->instruction.label].jumpIds[j] - 1
                                                );
                                ...
                                for (uint32_t j = i;
                                     index != INVALID_ID && block[index]->instruction.id < block[j]->instruction.id;
                                     j--) {
                                    if (function.is_valid_label(block[j]->instruction.label)) {
                                        if (function.labels[block[j]->instruction.label].jumpIds.back() >=
                                            block[j]->instruction.id) {
                                            index = INVALID_ID;
                                            break;
                                        }

                                        while (function.labels[block[j]->instruction.label].jumpIds.front() <
                                               block[index]->instruction.id) {
                                            if (!index) {
                                                index = INVALID_ID;
                                                break;
                                ...
                                if (index != INVALID_ID) {
                                    ...
                                    for (uint32_t j = index; j < targetIndex; j++) {
                                        switch (block[j]->type) {
                                                    ...
                                                    (block[j]->instruction.target == function.labels[targetLabel].target
                                                         ? !block[j]->assignment.variables.size() ||
                                                               *block[j]->assignment.variables.back().slotScope !=
                                                               ...
                                                               has_self_reference(
                                                                   block[i]->assignment.variables.back().slot,
                                                                   block[j]->assignment.expressions.back()
                                                               )
                                                         : block[j]->assignment.variables.size()))
                                                    break;
                                                conditionBuilder.add_node(
                                                    conditionBuilder.get_node_type(
                                                        block[j]->instruction.type, block[j]->condition.swapped
                                                    ),
                                                    block[j]->instruction.label,
                                                    function.get_label_from_id(block[j]->instruction.target),
                                            ...
                                            case AST_STATEMENT_ASSIGNMENT:
                                                ...
                                                switch (block[j]->type) {
                                                    case AST_STATEMENT_CONDITION:
                                                        if (block[j]->instruction.target !=
                                                                function.labels[targetLabel].target ||
                                                            block[j]->assignment.variables.size() ||
                                                            block[j]->assignment.expressions.size() != 1 ||
                                                            block[j]->assignment.expressions.back()->type !=
                                                                AST_EXPRESSION_VARIABLE ||
                                                            block[j]->assignment.expressions.back()->variable->type !=
                                                                AST_VARIABLE_SLOT ||
                                                            *block[j]
                                                                    ->assignment.expressions.back()
                                                                    ->variable->slotScope !=
                                                    ...
                                                    case AST_STATEMENT_BREAK:
                                                        if (function.is_valid_label(block[j]->instruction.label) ||
                                                            ...
                                                            block[j]->instruction.target !=
                                                                function.labels[targetLabel].target ||
                                                            block[j - 1]->assignment.expressions.back()->type !=
                                                                AST_EXPRESSION_CONSTANT ||
                                                            !get_constant_type(
                                                                block[j - 1]->assignment.expressions.back()
                                                            ))
                                                            break;
                                                        ...
                                                        switch (block[j - 1]
                                                            ...
                                                            case AST_CONSTANT_FALSE:
                                                                conditionBuilder.add_node(
                                                                    ConditionBuilder::Node::FALSY_TEST,
                                                                    block[j - 1]->instruction.label,
                                                                    function.get_label_from_id(
                                                                        block[j]->instruction.target
                                                                    ),
                                                                    &block[j - 1]->assignment.expressions
                                                                );
                                                            ...
                                                            case AST_CONSTANT_NUMBER:
                                                                conditionBuilder.add_node(
                                                                    ConditionBuilder::Node::TRUTHY_TEST,
                                                                    block[j - 1]->instruction.label,
                                                                    function.get_label_from_id(
                                                                        block[j]->instruction.target
                                                                    ),
                                    ...
                                    if (!hasBoolConstruct) {
                                        conditionBuilder.add_node(
                                            ConditionBuilder::Node::TRUTHY_TEST,
                                            block[i]->instruction.label,
                                            targetLabel,
                                            &block[i]->assignment.expressions
                                        );
                                    } else if (block[i - 3]->type == AST_STATEMENT_GOTO) {
                                        conditionBuilder.add_node(
                                            ConditionBuilder::Node::TRUTHY_TEST,
                                            block[i - 4]->instruction.label,
                                            targetLabel,
                                            &block[i - 4]->assignment.expressions
                                        );
                                    }
                                            ...
                                            switch (block[j]->type) {
                                                case AST_STATEMENT_CONDITION:
                                                    ...
                                                    function.remove_jump(
                                                        block[j]->instruction.id + 1, block[j]->instruction.target
                                                    );
                                                    if (block[j]->assignment.variables.size())
                                                        function.remove_jump(
                                                            block[j]->instruction.id, block[j]->instruction.id + 2
                                                        );
                                                    continue;
                                                case AST_STATEMENT_GOTO:
                                                case AST_STATEMENT_BREAK:
                                                    function.remove_jump(
                                                        block[j]->instruction.id, block[j]->instruction.target
                                                    );
                                                    continue;
                                                case AST_STATEMENT_ASSIGNMENT:
                                                    (*block[i]->assignment.variables.back().slotScope)->usages--;
                                ...
                                if ((*block[i]->assignment.variables.back().slotScope)->usages == 1 &&
                                    (i == block.size() - 1 || block[i + 1]->type != AST_STATEMENT_DECLARATION))
                                    break;
                                // TODO
                            }
                        ...
                        case AST_VARIABLE_TABLE_INDEX:
                            if (block[i]->assignment.variables.back().isMultres && i) {
                                ...
                                for (uint32_t j = i; j--;) {
                                    if (block[j]->type == AST_STATEMENT_ASSIGNMENT &&
                                        ...
                                        block[j]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                                        block[j]->assignment.variables.back().slot ==
                                            block[i]->assignment.variables.back().table->variable->slot) {
                                        if (block[j]->assignment.isTableConstructor)
                                            index = j;
                                        break;
                                    }
                                ...
                                if (index != INVALID_ID) {
                                    ...
                                    assert(
                                        !block[index]->assignment.expressions.back()->table->multresField,
                                        "Table constructor has multiple multres fields",
                                        bytecode.filePath,
                                        DEBUG_INFO
                                    );
                                    block[index]->assignment.expressions.back()->table->multresIndex =
                                        block[i]->assignment.variables.back().multresIndex;
                                    block[index]->assignment.expressions.back()->table->multresField =
                                        block[i]->assignment.expressions.back();
                            ...
                            if (i && !function.is_valid_label(block[i]->instruction.label) &&
                                block[i - 1]->type == AST_STATEMENT_ASSIGNMENT &&
                                block[i - 1]->assignment.variables.size() == 1 &&
                                block[i - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                                block[i - 1]->assignment.variables.back().slot ==
                                ...
                                if (block[i - 1]->assignment.isTableConstructor &&
                                    ...
                                    if (block[i]->assignment.variables.back().isMultres) {
                                        block[i - 1]->assignment.expressions.back()->table->multresIndex =
                                            block[i]->assignment.variables.back().multresIndex;
                                        block[i - 1]->assignment.expressions.back()->table->multresField =
                                            block[i]->assignment.expressions.back();
                                    } else {
                                        if (block[i]->assignment.variables.back().tableIndex->type ==
                                                AST_EXPRESSION_CONSTANT &&
                                            block[i]->assignment.variables.back().tableIndex->constant->type ==
                                                AST_CONSTANT_STRING) {
                                            for (uint32_t j = block[i - 1]
                                                                  ->assignment.expressions.back()
                                                                  ->table->constants.fields.size();
                                                 j--;) {
                                                if (block[i - 1]
                                                            ->assignment.expressions.back()
                                                            ->table->constants.fields[j]
                                                            .key->constant->type != AST_CONSTANT_STRING ||
                                                    block[i - 1]
                                                            ->assignment.expressions.back()
                                                            ->table->constants.fields[j]
                                                            .key->constant->string != block[i]
                                                                                          ->assignment.variables.back()
                                                                                          .tableIndex->constant->string)
                                                    continue;
                                                if (block[i - 1]
                                                        ->assignment.expressions.back()
                                                        ->table->constants.fields[j]
                                                        .value->constant->type == AST_CONSTANT_NIL)
                                                    block[i - 1]
                                                        ->assignment.expressions.back()
                                                        ->table->constants.fields.erase(
                                                            block[i - 1]
                                                                ->assignment.expressions.back()
                                                                ->table->constants.fields.begin() +
                                                            j
                                                        );
                                                break;
                                            }
                                        ...
                                        block[i - 1]->assignment.expressions.back()->table->fields.back().key =
                                            block[i]->assignment.variables.back().tableIndex;
                                        block[i - 1]->assignment.expressions.back()->table->fields.back().value =
                                            block[i]->assignment.expressions.back();
                                    }
...
void Ast::lower_test_copy_conditions(std::vector<Statement*>& block) {
    for (Statement* statement : block) {
        if (statement->type != AST_STATEMENT_CONDITION || !statement->assignment.variables.size())
            continue;
...
void Ast::eliminate_conditions(Function& function, std::vector<Statement*>& block, BlockInfo* const& previousBlock) {
    ...
    for (uint32_t i = block.size(); i--;) {
        ...
        switch (block[i]->type) {
            case AST_STATEMENT_CONDITION:
                ...
                for (uint32_t j = function.labels[targetLabel].jumpIds.size(); j--;) {
                    if (function.labels[targetLabel].jumpIds[j] >= function.labels[targetLabel].target)
                        continue;
                            ...
                            if (!block[index]->assignment.variables.size()) {
                                if (targetLabel == extendedTargetLabel ||
                                    (block[index]->assignment.expressions.size() == 1 &&
                                     ...
                                     block[index]->assignment.expressions.back()->variable->type ==
                                         AST_VARIABLE_SLOT)) {
                                    index = INVALID_ID;
                                    continue;
                                }
        ...
        if (i >= 3 && block[i]->type == AST_STATEMENT_ASSIGNMENT &&
            block[i]->assignment.expressions.back()->type == AST_EXPRESSION_CONSTANT &&
            block[i]->assignment.expressions.back()->constant->type == AST_CONSTANT_TRUE &&
            (block[i - 1]->type == AST_STATEMENT_GOTO || block[i - 1]->type == AST_STATEMENT_BREAK) &&
            !function.is_valid_label(block[i - 1]->instruction.label) &&
            block[i - 1]->instruction.type == Bytecode::BC_OP_JMP &&
            block[i - 1]->instruction.target == function.labels[targetLabel].target &&
            block[i - 2]->type == AST_STATEMENT_ASSIGNMENT &&
            ...
            block[i - 2]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
            block[i - 2]->assignment.variables.back().slot ==
                block[assignmentIndex]->assignment.variables.back().slot) {
            switch (block[i - 3]->type) {
                case AST_STATEMENT_CONDITION:
                    if (block[i - 3]->assignment.expressions.size() == 2 &&
                        block[i - 3]->instruction.target == block[i]->instruction.id)
            ...
            if (hasBoolConstruct) {
                ...
                if (function.is_valid_label(block[i]->instruction.label)) {
                    for (uint32_t j = function.labels[block[i]->instruction.label].jumpIds.size(); j--;) {
                        targetIndex =
                            get_block_index_from_id(block, function.labels[block[i]->instruction.label].jumpIds[j] - 1);
                ...
                if (hasBoolConstruct && function.is_valid_label(block[i - 2]->instruction.label)) {
                    for (uint32_t j = function.labels[block[i - 2]->instruction.label].jumpIds.size(); j--;) {
                        targetIndex = get_block_index_from_id(
                            block, function.labels[block[i - 2]->instruction.label].jumpIds[j] - 1
                        );
        ...
        hasEndAssignment = hasBoolConstruct ? block[i - 3]->type == AST_STATEMENT_CONDITION ||
                                                  block[i - 4]->type == AST_STATEMENT_ASSIGNMENT
                                            : block[i]->type == AST_STATEMENT_ASSIGNMENT;
        targetIndex = hasBoolConstruct
                          ? (block[i - 3]->type == AST_STATEMENT_GOTO ? i - (hasEndAssignment ? 4 : 3) : i - 2)
                          : (hasEndAssignment ? i : i + 1);
        ...
        for (uint32_t j = function.labels[targetLabel].jumpIds.size(); j--;) {
            ...
            if (function.labels[targetLabel].jumpIds[j] < block[index]->instruction.id) {
                ...
                if (hasBoolConstruct && index == i - 2 && !function.is_valid_label(block[i]->instruction.label)) {
                    index = get_block_index_from_id(
                        block, function.labels[block[i - 2]->instruction.label].jumpIds.front() - 1
                    );
                    if (index == INVALID_ID)
                        index = i - 2;
                }
            ...
            for (uint32_t k = i; index != INVALID_ID && block[index]->instruction.id < block[k]->instruction.id; k--) {
                if (function.is_valid_label(block[k]->instruction.label)) {
                    ...
                    while (function.labels[block[k]->instruction.label].jumpIds.front() <
                           block[index]->instruction.id) {
                        if (!index) {
                            index = INVALID_ID;
                            break;
            ...
            switch (block[index]->type) {
                case AST_STATEMENT_GOTO:
                case AST_STATEMENT_BREAK:
                    if (block[index]->instruction.target == function.labels[targetLabel].target && index)
                        index--;
            }
                ...
                switch (block[k]->type) {
                    case AST_STATEMENT_CONDITION:
                        if (block[k]->assignment.variables.size()) {
                            if (block[k]->instruction.target == function.labels[targetLabel].target &&
                                block[k]->assignment.variables.back().slot ==
                                    block[assignmentIndex]->assignment.variables.back().slot)
                                continue;
                        } else if (
                            ...
                            block[k]->assignment.expressions.back()->variable->slot ==
                                block[assignmentIndex]->assignment.variables.back().slot) {
                            continue;
                        } else if (
                    ...
                    case AST_STATEMENT_ASSIGNMENT:
                        if (block[k]->assignment.variables.size() == 1 &&
                            ...
                            get_constant_type(block[k]->assignment.expressions.back()) && ++k != targetIndex &&
        ...
        if (previousValidIndex == INVALID_ID)
            continue;
        ...
        for (uint32_t j = index; j < targetIndex; j++) {
            switch (block[j]->type) {
                case AST_STATEMENT_CONDITION:
                    conditionBuilder.add_node(
                        conditionBuilder.get_node_type(block[j]->instruction.type, block[j]->condition.swapped),
                        block[j]->instruction.label,
                        hasEndAssignment || block[j]->assignment.variables.size() ||
                                (block[j]->instruction.target == function.labels[targetLabel].target
                                     ? targetLabel != extendedTargetLabel ||
                                           (block[j]->assignment.expressions.size() == 1 &&
                                            ...
                                            block[j]->assignment.expressions.back()->variable->type ==
                                                AST_VARIABLE_SLOT &&
                                            block[j]->assignment.expressions.back()->variable->slot ==
                                                block[assignmentIndex]->assignment.variables.back().slot)
                                     : block[j]->instruction.target != function.labels[extendedTargetLabel].target)
                            ? function.get_label_from_id(block[j]->instruction.target)
                            : function.labels.size(),
                        &block[j]->assignment.expressions,
                        block[j]->instruction.target == function.labels[targetLabel].target && !hasEndAssignment &&
                            block[j]->assignment.expressions.size() == 1 &&
                            block[j]->assignment.expressions.back()->type == AST_EXPRESSION_VARIABLE &&
                    ...
                    switch (block[j]->assignment.expressions.back()->constant->type) {
                        ...
                        case AST_CONSTANT_FALSE:
                            conditionBuilder.add_node(
                                ConditionBuilder::Node::FALSY_TEST,
                                block[j]->instruction.label,
                                function.get_label_from_id(block[j + 1]->instruction.target),
                                &block[j]->assignment.expressions
                            );
                            break;
                        case AST_CONSTANT_TRUE:
        ...
        if (hasEndAssignment) {
            ...
            conditionBuilder.add_node(
                ConditionBuilder::Node::TRUTHY_TEST, function.labels.size(), targetLabel, &expressions
            );
        }
        ...
        for (uint32_t j = index; j <= i; j++) {
            switch (block[j]->type) {
                ...
                case AST_STATEMENT_ASSIGNMENT:
                    if (*block[j]->assignment.variables.back().slotScope !=
                        *block[assignmentIndex]->assignment.variables.back().slotScope) {
                        (*block[assignmentIndex]->assignment.variables.back().slotScope)->usages +=
                            (*block[j]->assignment.variables.back().slotScope)->usages;
                        if ((*block[j]->assignment.variables.back().slotScope)->scopeBegin <
                            (*block[assignmentIndex]->assignment.variables.back().slotScope)->scopeBegin)
                            (*block[assignmentIndex]->assignment.variables.back().slotScope)->scopeBegin =
                                (*block[j]->assignment.variables.back().slotScope)->scopeBegin;
                        if ((*block[j]->assignment.variables.back().slotScope)->scopeEnd >
                            (*block[assignmentIndex]->assignment.variables.back().slotScope)->scopeEnd)
                            (*block[assignmentIndex]->assignment.variables.back().slotScope)->scopeEnd =
                                (*block[j]->assignment.variables.back().slotScope)->scopeEnd;
                        *block[j]->assignment.variables.back().slotScope =
                            *block[assignmentIndex]->assignment.variables.back().slotScope;
                        if (block[j]->assignment.variables.back().slotScope !=
                            block[assignmentIndex]->assignment.variables.back().slotScope)
        ...
        block[i]->type = AST_STATEMENT_ASSIGNMENT;
        block[i]->instruction.label = block[index]->instruction.label;
        if ((*block[i]->assignment.variables.back().slotScope)->scopeBegin >= block[index]->instruction.id)
    ...
    for (uint32_t i = block.size(); i--;) {
        switch (block[i]->type) {
            case AST_STATEMENT_CONDITION:
                ...
                for (uint32_t j = index; j <= i; j++) {
                    if (function.is_valid_label(block[j]->instruction.label)) {
                        if (function.labels[block[j]->instruction.label].jumpIds.front() <
                                block[index]->instruction.id ||
                            ...
                            for (uint32_t k = index; k < j && block[k]->instruction.target > block[k]->instruction.id &&
                ...
                {
                    ConditionBuilder conditionBuilder(
                        ConditionBuilder::STATEMENT, *this, INVALID_ID, targetLabel, extendedTargetLabel
                    );
                ...
                if (i && block[i]->instruction.type == Bytecode::BC_OP_JMP &&
                    block[i]->assignment.expressions.back()->type == AST_EXPRESSION_CONSTANT &&
...
void Ast::build_multi_assignment(Function& function, std::vector<Statement*>& block) {
    ...
    for (uint32_t i = block.size(); i--;) {
        switch (block[i]->type) {
            case AST_STATEMENT_ASSIGNMENT:
                if (block[i]->assignment.variables.size() >= 2) {
                    if (i + block[i]->assignment.variables.size() >= block.size())
                        continue;
                    ...
                    for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
                        if ((*block[i]->assignment.variables[j].slotScope)->usages == 1 &&
                            !function.is_valid_label(
                                block[i + block[i]->assignment.variables.size() - j]->instruction.label
                            ) &&
                            ...
                            (block[i + block[i]->assignment.variables.size() - j]->assignment.variables.back().type !=
                                 AST_VARIABLE_TABLE_INDEX ||
                             (block[i + block[i]->assignment.variables.size() - j]
                                      ->assignment.variables.back()
                                      .table->type == AST_EXPRESSION_VARIABLE &&
                              block[i + block[i]->assignment.variables.size() - j]
                                      ->assignment.variables.back()
                                      .table->variable->type == AST_VARIABLE_SLOT &&
                              (get_constant_type(
                                   block[i + block[i]->assignment.variables.size() - j]
                                       ->assignment.variables.back()
                                       .tableIndex
                               ...
                               (block[i + block[i]->assignment.variables.size() - j]
                                        ->assignment.variables.back()
                                        .tableIndex->type == AST_EXPRESSION_VARIABLE &&
                                block[i + block[i]->assignment.variables.size() - j]
                                        ->assignment.variables.back()
                                        .tableIndex->variable->type == AST_VARIABLE_SLOT)))) &&
                            block[i + block[i]->assignment.variables.size() - j]->assignment.expressions.size() == 1 &&
                            block[i + block[i]->assignment.variables.size() - j]->assignment.expressions.back()->type ==
                                AST_EXPRESSION_VARIABLE &&
                            block[i + block[i]->assignment.variables.size() - j]
                                    ->assignment.expressions.back()
                                    ->variable->type == AST_VARIABLE_SLOT &&
                            block[i + block[i]->assignment.variables.size() - j]
                                    ->assignment.expressions.back()
                                    ->variable->slotScope == block[i]->assignment.variables[j].slotScope)
                            continue;
                    ...
                    for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
                        function.slotScopeCollector.remove_scope(
                            block[i]->assignment.variables[j].slot, block[i]->assignment.variables[j].slotScope
                        );
            ...
            case AST_STATEMENT_FUNCTION_CALL:
                ...
                if (block[i]->type == AST_STATEMENT_FUNCTION_CALL ||
                    ...
                    while (index && !function.is_valid_label(block[index]->instruction.label) &&
                           block[index - 1]->type == AST_STATEMENT_ASSIGNMENT &&
                           block[index - 1]->assignment.variables.size() == 1 &&
                           block[index - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                ...
                if (index && i + 1 < block.size() && !function.is_valid_label(block[index]->instruction.label) &&
                    !function.is_valid_label(block[i + 1]->instruction.label) &&
                    block[index - 1]->type == AST_STATEMENT_ASSIGNMENT &&
                    block[index - 1]->assignment.variables.size() == 1 &&
                    block[index - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
                    ...
                    block[i + 1]->assignment.expressions.back()->variable->slotScope ==
                        block[index - 1]->assignment.variables.back().slotScope) {
                    if (block[i]->type == AST_STATEMENT_ASSIGNMENT) {
                        switch (block[i]->assignment.variables.back().type) {
                            case AST_VARIABLE_SLOT:
                                if (!(*block[i]->assignment.variables.back().slotScope)->usages &&
                                    !block[i]->assignment.forwardDeclaration) {
                                    function.slotScopeCollector.remove_scope(
                                        block[i]->assignment.variables.back().slot,
                                        block[i]->assignment.variables.back().slotScope
                            ...
                            case AST_VARIABLE_TABLE_INDEX:
                                if (index == i &&
                                    (block[i]->assignment.variables.back().table->type != AST_EXPRESSION_VARIABLE ||
                                     ...
                                     (!get_constant_type(block[i]->assignment.variables.back().tableIndex) &&
                                      (block[i]->assignment.variables.back().tableIndex->type !=
                                           AST_EXPRESSION_VARIABLE ||
                                       block[i]->assignment.variables.back().tableIndex->variable->type !=
                                           AST_VARIABLE_SLOT))))
                                    continue;
                                break;
                        }
                    ...
                    while (i != index) {
                        ...
                        block[i + 1]->assignment.expressions.emplace(
                            block[i + 1]->assignment.expressions.begin(), block[i]->assignment.expressions.back()
                        );
                        block.erase(block.begin() + i);
                    }
                ...
                if (block[i]->type == AST_STATEMENT_FUNCTION_CALL &&
                    ...
                    assert(
                        i && !function.is_valid_label(block[i]->instruction.label) &&
                            block[i - 1]->type == AST_STATEMENT_ASSIGNMENT &&
                            block[i - 1]->assignment.variables.size() == 1,
                        "Unable to eliminate vararg with zero returns",
                        bytecode.filePath,
                        DEBUG_INFO
                    );
        ...
        while (i && i + 1 < block.size() && !function.is_valid_label(block[i]->instruction.label) &&
               !function.is_valid_label(block[i + 1]->instruction.label) &&
               block[i - 1]->type == AST_STATEMENT_ASSIGNMENT && block[i - 1]->assignment.variables.size() == 1 &&
               block[i - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
               (*block[i - 1]->assignment.variables.back().slotScope)->usages == 1 &&
            ...
            block[i]->assignment.expressions.emplace(
                block[i]->assignment.expressions.begin(), block[i - 1]->assignment.expressions.back()
            );
            block[i]->assignment.variables.emplace(
                block[i]->assignment.variables.begin(), block[i + 1]->assignment.variables.back()
            );
            block[i]->instruction.label = block[i - 1]->instruction.label;
            block.erase(block.begin() + i - 1);
            block.erase(block.begin() + i);
        ...
        for (uint8_t j = block[i]->assignment.variables.size();
             j-- && i && !function.is_valid_label(block[i]->instruction.label) &&
             block[i - 1]->type == AST_STATEMENT_ASSIGNMENT && block[i - 1]->assignment.variables.size() == 1 &&
             block[i - 1]->assignment.variables.back().type == AST_VARIABLE_SLOT &&
             (*block[i - 1]->assignment.variables.back().slotScope)->usages == 1;) {
            if (block[i]->assignment.variables[j].type != AST_VARIABLE_TABLE_INDEX)
                continue;
            ...
            if (block[i]->assignment.variables[j].tableIndex->type == AST_EXPRESSION_VARIABLE &&
                ...
                block[i]->assignment.variables[j].tableIndex->variable->slotScope ==
                    block[i - 1]->assignment.variables.back().slotScope) {
                function.slotScopeCollector.remove_scope(
                    block[i - 1]->assignment.variables.back().slot, block[i - 1]->assignment.variables.back().slotScope
                );
...
void Ast::build_if_statements_from_map(
    Function& function,
    ...
    for (uint32_t i = 0; i < block.size(); i++) {
        switch (block[i]->type) {
            ...
            case AST_STATEMENT_CONDITION:
                ...
                if (block[i]->type == AST_STATEMENT_CONDITION && block[i]->block.size() &&
                    ...
                    block[i + 1]->block.insert(
                        block[i + 1]->block.begin(), block.begin() + i + 2, block.begin() + index + 2
                    );
                    block.erase(block.begin() + i + 2, block.begin() + index + 2);
                    function.remove_jump(
                        block[i]->block.back()->instruction.id, block[i]->block.back()->instruction.target
                    );
                ...
                if (block[i]->type == AST_STATEMENT_GOTO)
...
void Ast::build_if_statements(Function& function, std::vector<Statement*>& block, BlockInfo* const& previousBlock) {
    const auto build_if_false_statements = [&](std::vector<Statement*>& block,
                                               BlockInfo* const& previousBlock) -> void {
        BlockInfo blockInfo = {.block = block, .previousBlock = previousBlock};
        uint32_t index, targetLabel;

        for (uint32_t i = block.size(); i--;) {
            if (block[i]->type != AST_STATEMENT_GOTO || block[i]->instruction.type == Bytecode::BC_OP_LOOP)
                continue;
            targetLabel = INVALID_ID;
            ...
            for (index = i; index < block.size(); index++) {
                ...
                targetLabel = get_label_from_next_statement(function, blockInfo, false, false);
                if (targetLabel == INVALID_ID || function.labels[targetLabel].target != block[i]->instruction.target)
                    targetLabel = get_label_from_next_statement(function, blockInfo, true, false);
                if (targetLabel == INVALID_ID)
                    continue;
                if (function.labels[targetLabel].target == block[i]->instruction.target &&
                    is_valid_block(function, blockInfo, block[i]->instruction.id + 1))
                    break;
                targetLabel = INVALID_ID;
            }
        ...
        for (uint32_t i = block.size(); i--;) {
            ...
            if (block[i]->block.size() && block[i]->block.back()->type == AST_STATEMENT_GOTO &&
                ...
                for (index = i; index < block.size(); index++) {
                    ...
                    targetLabel = get_label_from_next_statement(function, blockInfo, false, false);
                    if (targetLabel == INVALID_ID ||
                        function.labels[targetLabel].target != block[i]->block.back()->instruction.target)
                        targetLabel = get_label_from_next_statement(function, blockInfo, true, false);
                    if (targetLabel == INVALID_ID)
                        continue;
                    if (function.labels[targetLabel].target == block[i]->block.back()->instruction.target &&
                        is_valid_block(function, blockInfo, block[i]->block.back()->instruction.id + 1))
                        break;
                    targetLabel = INVALID_ID;
                }
    ...
    for (uint32_t i = 0; i < block.size(); i++) {
        if (indexes.size() && (i == indexes.back() || block[i]->type != AST_STATEMENT_CONDITION)) {
            blockInfo.index = i;
            targetLabel = get_label_from_next_statement(function, blockInfo, false, false);
            if (targetLabel == INVALID_ID ||
            ...
            if (targetLabel != INVALID_ID &&
                ...
                is_valid_block(
                    function,
                    blockInfo,
                    block[indexes.back()]->instruction.id +
                        (block[indexes.back()]->type == AST_STATEMENT_CONDITION ? 2 : 1)
                )) {
                offsetMap.emplace(block[indexes.back()], i - indexes.back());

                if (i - indexes.back() && block[indexes.back()]->type == AST_STATEMENT_CONDITION &&
                    block[i]->type == AST_STATEMENT_GOTO && block[i]->instruction.type != Bytecode::BC_OP_LOOP) {
                    function.remove_jump(
                        block[indexes.back()]->instruction.id, block[indexes.back()]->instruction.target
                    );
        ...
        switch (block[i]->type) {
            case AST_STATEMENT_GOTO:
                if (block[i]->instruction.type == Bytecode::BC_OP_LOOP)
                    continue;
            case AST_STATEMENT_CONDITION:
                if (offsetMap.contains(block[i]))
                    continue;
    ...
    if (indexes.size() == 1 && block[indexes.back()]->type == AST_STATEMENT_GOTO &&
        indexes.back() == block.size() - 1 && previousBlock &&
        previousBlock->block[previousBlock->index]->type == AST_STATEMENT_LOOP)
        indexes.pop_back();
    if (!indexes.size())
        return build_if_statements_from_map(function, block, previousBlock, offsetMap);

    for (uint32_t i = indexes.size(); i--;) {
        if (offsetMap.contains(block[indexes[i]]))
            function.add_jump(block[indexes[i]]->instruction.id, block[indexes[i]]->instruction.target);
    }
...
void Ast::clean_up(Function& function) {
    if (function.hasDebugInfo) {
        ...
        for (uint32_t i = function.parameterNames.size(); i--;) {
            function.parameterNames[i] =
                "arg_" + std::to_string(minimizeDiffs ? function.level : function.id) + "_" + std::to_string(i);
            (*function.slotScopeCollector.slotInfos[i].activeSlotScope)->name = function.parameterNames[i];
        }
    }
    ...
    for (uint32_t i = 0, labelCounter = 0; i < function.labels.size(); i++) {
        if (!function.labels[i].jumpIds.size())
            continue;
        function.labels[i].name = "label_" + std::to_string(minimizeDiffs ? function.level : function.id) + "_" +
                                  std::to_string(labelCounter);
        labelCounter++;
    }
}

void Ast::clean_up_block(
    Function& function,
    std::vector<Statement*>& block,
        ...
        switch (block[i]->type) {
            ...
            case AST_STATEMENT_GENERIC_FOR:
                ...
                if (function.hasDebugInfo) {
                    ...
                    for (uint8_t j = 0; j < block[i]->assignment.variables.size(); j++) {
                        (*block[i]->assignment.variables[j].slotScope)->name =
                            "iter_" + std::to_string(minimizeDiffs ? function.level : function.id) + "_" +
                            std::to_string(iteratorCounter);
                        iteratorCounter++;
                    }
                }
                ...
                continue;
            case AST_STATEMENT_LOOP:
                for (std::vector<Statement*>* currentBlock = &block[i]->block; currentBlock->size();
                     currentBlock = &currentBlock->back()->block) {
                    if (currentBlock->back()->type == AST_STATEMENT_DECLARATION)
                        continue;
                    if (currentBlock->back()->type != AST_STATEMENT_GOTO ||
                        currentBlock->back()->instruction.target != block[i]->instruction.id ||
                        function.is_valid_label(currentBlock->back()->instruction.label) || currentBlock->size() < 2)
                        break;

                    switch ((*currentBlock)[currentBlock->size() - 2]->type) {
                        case AST_STATEMENT_BREAK:
                            function.remove_jump(
                                currentBlock->back()->instruction.id, currentBlock->back()->instruction.target
                            );
                            function.remove_jump(
                                (*currentBlock)[currentBlock->size() - 2]->instruction.id,
                                (*currentBlock)[currentBlock->size() - 2]->instruction.target
                            );
                        ...
                        case AST_STATEMENT_IF:
                            if ((*currentBlock)[currentBlock->size() - 2]->block.size() != 1 ||
                                (*currentBlock)[currentBlock->size() - 2]->block.back()->type != AST_STATEMENT_BREAK)
                                break;
                            function.remove_jump(
                                currentBlock->back()->instruction.id, currentBlock->back()->instruction.target
                            );
                            function.remove_jump(
                                (*currentBlock)[currentBlock->size() - 2]->block.back()->instruction.id,
                                (*currentBlock)[currentBlock->size() - 2]->block.back()->instruction.target
                            );
                            block[i]->type = AST_STATEMENT_REPEAT;
                            block[i]->assignment.expressions.emplace_back(
                                (*currentBlock)[currentBlock->size() - 2]->assignment.expressions.back()
                            );
                            (*currentBlock)[currentBlock->size() - 2]->type = AST_STATEMENT_EMPTY;
                            currentBlock->erase(currentBlock->begin() + currentBlock->size() - 1);
                            break;
                ...
                if (block[i]->type == AST_STATEMENT_LOOP) {
                    if (block[i]->block.size() && block[i]->block.back()->type == AST_STATEMENT_GOTO) {
                        if (block[i]->instruction.id == block[i]->block.back()->instruction.target) {
                            ...
                            for (uint32_t j = i;
                                 j-- && block[j]->type == AST_STATEMENT_IF && !block[j]->block.size();) {
                                if (!function.is_valid_label(block[j]->instruction.label))
                                    continue;
                                if (function.is_valid_label(block[i]->instruction.label) ||
                                    function.labels[block[j]->instruction.label].target !=
                                ...
                                for (Expression* expression; i != j; i--) {
                                    ...
                                    if (block[i]->assignment.expressions.size()) {
                                        block[i - 1]->assignment.expressions.back() =
                                            new_expression(AST_EXPRESSION_BINARY_OPERATION);
                                        block[i - 1]->assignment.expressions.back()->binaryOperation->type =
                                            AST_BINARY_AND;
                                        block[i - 1]->assignment.expressions.back()->binaryOperation->rightOperand =
                                            block[i]->assignment.expressions.back();
                                        block[i - 1]->assignment.expressions.back()->binaryOperation->leftOperand =
                                            expression;
                                        block[i]->assignment.expressions.back() =
                                            block[i - 1]->assignment.expressions.back();
                                    } else {
                            ...
                            previousBlock && previousBlock->block[previousBlock->index]->type == AST_STATEMENT_IF &&
                            !function.is_valid_label(block[i]->instruction.label)) {
                            if (function.is_valid_label(
                                    previousBlock->block[previousBlock->index]->instruction.label
                                )) {
                                if (function.labels[previousBlock->block[previousBlock->index]->instruction.label]
                                        .target == block[i]->block.back()->instruction.target) {
                                    function.remove_jump(
                                    ...
                                    block[i]->type = AST_STATEMENT_WHILE;
                                    block[i]->assignment.expressions.emplace_back(
                                        previousBlock->block[previousBlock->index]->assignment.expressions.back()
                                    );
                                    block[i]->instruction.label =
                                        previousBlock->block[previousBlock->index]->instruction.label;
                                    block[i]->block.back()->type = AST_STATEMENT_EMPTY;
                                ...
                                for (uint32_t j = previousBlock->index - 1;
                                     j-- && previousBlock->block[j]->type == AST_STATEMENT_IF &&
                                     !previousBlock->block[j]->block.size();) {
                                    if (!function.is_valid_label(previousBlock->block[j]->instruction.label))
                                        continue;
                                    if (function.labels[previousBlock->block[j]->instruction.label].target !=
                                        block[i]->block.back()->instruction.target)
                                        break;
                                    function.remove_jump(
                                        block[i]->block.back()->instruction.id,
                                    ...
                                    block[i]->type = AST_STATEMENT_WHILE;
                                    block[i]->assignment.expressions.resize(
                                        1, new_expression(AST_EXPRESSION_BINARY_OPERATION)
                                    );
                                    ...
                                    for (Expression* expression; j != previousBlock->index - 1; j++) {
                                        ...
                                        expression->binaryOperation->rightOperand = new_primitive(2);
                                        expression->binaryOperation->leftOperand =
                                            previousBlock->block[j]->assignment.expressions.back();

                                        if (block[i]->assignment.expressions.back()->binaryOperation->leftOperand) {
                                            previousBlock->block[j]->assignment.expressions.back() =
                                                new_expression(AST_EXPRESSION_BINARY_OPERATION);
                                            previousBlock->block[j]
                                                ->assignment.expressions.back()
                                                ->binaryOperation->type = AST_BINARY_AND;
                                            previousBlock->block[j]
                                                ->assignment.expressions.back()
                                                ->binaryOperation->rightOperand = expression;
                                            previousBlock->block[j]
                                                ->assignment.expressions.back()
            ...
            case AST_STATEMENT_ASSIGNMENT:
                if (block[i]->assignment.variables.size() == 1 &&
                    ...
                    block[i]->assignment.variables.back().tableIndex =
                        block[i]->assignment.expressions.back()->table->fields.back().key;
                    block[i]->assignment.expressions.back() =
                        block[i]->assignment.expressions.back()->table->fields.back().value;
                    block[i]->assignment.variables.back().table->table->fields.pop_back();
                    continue;
                }

                for (uint32_t j = 0; j < block[i]->assignment.variables.size(); j++) {
                    if (block[i]->assignment.variables[j].type != AST_VARIABLE_SLOT ||
                        (*block[i]->assignment.variables[j].slotScope)->name.size()) {
                        block[i]->assignment.forwardDeclaration = true;
                        continue;
                    }
                    ...
                    (*block[i]->assignment.variables[j].slotScope)->name =
                        "var_" + std::to_string(minimizeDiffs ? function.level : function.id) + "_" +
                ...
                if (declarations.size()) {
                    ...
                    for (uint8_t j = 0; j < declarations.size(); j++) {
                        ...
                        for (BlockInfo* currentBlockInfo = &blockInfo; currentBlockInfo->previousBlock;) {
                            ...
                            if (currentBlockInfo->index == currentBlockInfo->block.size() - 1)
                                continue;

                            switch (currentBlockInfo->block[currentBlockInfo->index]->type) {
                                case AST_STATEMENT_IF:
                                    if (currentBlockInfo->block[currentBlockInfo->index + 1]->type ==
                                        AST_STATEMENT_ELSE) {
                                        if ((*declarations[j]->slotScope)->scopeEnd <=
                                            currentBlockInfo->block[currentBlockInfo->index]
                                                ->block.back()
                                                ->instruction.id)
                                            break;
                                ...
                                case AST_STATEMENT_ELSE:
                                    switch (currentBlockInfo->block[currentBlockInfo->index + 1]->type) {
                                        ...
                                        case AST_STATEMENT_GOTO:
                                        case AST_STATEMENT_BREAK:
                                            if (currentBlockInfo->block[currentBlockInfo->index + 1]
                                                    ->instruction.type == Bytecode::BC_OP_JMP) {
                                                if ((*declarations[j]->slotScope)->scopeEnd <
                                                    currentBlockInfo->block[currentBlockInfo->index + 1]
                                                        ->instruction.id)
                                                    break;
                                                declarationTarget =
                                                    &currentBlockInfo->block
                                                         [currentBlockInfo->index -
                                                          (currentBlockInfo->block[currentBlockInfo->index]->type ==
                                                                   AST_STATEMENT_ELSE
                                                               ? 2
                                                               : 1)];
                                                block[i]->assignment.forwardDeclaration = true;
                                        ...
                                        default:
                                            if ((*declarations[j]->slotScope)->scopeEnd <
                                                function
                                                    .labels[currentBlockInfo->block[currentBlockInfo->index + 1]
                                                                ->instruction.label]
                                                    .target)
                                                break;
                                            declarationTarget =
                ...
                if (i != block.size() - 1 && block[i + 1]->type == AST_STATEMENT_ELSE) {
                    ...
                    if (block[i - 1]->type == AST_STATEMENT_EMPTY) {
                        ...
                        block.insert(
                            block.begin() + i + 1,
                            block[i]->block.begin(),
                            block[i]->block.begin() + block[i]->block.size()
                        );
                        block[i]->block.clear();
    ...
    for (uint32_t i = block.size(); i--;) {
                ...
                if (function.labels[block[j]->instruction.label].jumpIds.front() < block[i]->instruction.target) {
                    ...
                    if (block[i]->block.size() && block[i]->block.back()->type == AST_STATEMENT_DO) {
                        ...
                        block[i]->block.insert(
                            block[i]->block.begin() + block[i]->block.size() - 1,
                            block[i]->block.back()->block.begin(),
                            block[i]->block.back()->block.begin() + block[i]->block.back()->block.size()
                        );
        ...
        if (i == block.size() - 1) {
            block.reserve(block.size() + block[i]->block.size());
            block.insert(
                block.begin() + block.size(), block[i]->block.begin(), block[i]->block.begin() + block[i]->block.size()
            );
            block[i]->block.clear();
            block[i]->block.shrink_to_fit();
            continue;
...
uint32_t Ast::get_block_index_from_id(const std::vector<Statement*>& block, const uint32_t& id) {
    for (uint32_t i = block.size();
         i-- && (block[i]->instruction.id == INVALID_ID || block[i]->instruction.id >= id);) {
        if (block[i]->instruction.id == id)
            return i;
    }
...
uint32_t Ast::get_extended_id_from_statement(Statement* const& statement) {
    switch (statement->type) {
        case AST_STATEMENT_GOTO:
        case AST_STATEMENT_BREAK:
            if (statement->instruction.type == Bytecode::BC_OP_JMP)
...
uint32_t Ast::get_label_from_next_statement(
    Function& function, const BlockInfo& blockInfo, const bool& returnExtendedLabel, const bool& excludeDeclaration
) {
    if (blockInfo.index == blockInfo.block.size() - 1)
        return blockInfo.previousBlock
                   ? get_label_from_next_statement(function, *blockInfo.previousBlock, returnExtendedLabel, false)
                   : INVALID_ID;
    ...
    switch (statement->type) {
        ...
        case AST_STATEMENT_GOTO:
        case AST_STATEMENT_BREAK:
            if (returnExtendedLabel && statement->instruction.type == Bytecode::BC_OP_JMP)
                return function.get_label_from_id(statement->instruction.target);
    }
...
bool Ast::is_valid_block(Function& function, const BlockInfo& blockInfo, const uint32_t& blockBegin) {
    if (blockInfo.index == blockInfo.block.size() - 1)
        return blockInfo.previousBlock ? is_valid_block(function, *blockInfo.previousBlock, blockBegin) : true;
    const uint32_t blockEnd = blockInfo.block[blockInfo.index + 1]->instruction.label != INVALID_ID
                                  ? function.labels[blockInfo.block[blockInfo.index + 1]->instruction.label].target
                                  : blockInfo.block[blockInfo.index + 1]->instruction.id;
    return blockEnd == INVALID_ID
               ? true
               : (blockEnd > blockBegin ? function.is_valid_block_range(blockBegin, blockEnd - 1, false) : true);
}

void Ast::check_valid_name(Constant* const& constant) {
    static const std::string KEYWORDS[] = {"and", "break",    "do",     "else", "elseif", "end",   "false",
                                           "for", "function", "if",     "in",   "local",  "nil",   "not",
                                           "or",  "repeat",   "return", "then", "true",   "until", "while"};

    if (!constant->string.size() || constant->string.front() < 'A')
        return;

    for (uint32_t i = constant->string.size(); i--;) {
        if (constant->string[i] < '0')
            return;
...
void Ast::check_special_number(Expression* const& expression, const bool& isCdata) {
    ...
    if ((rawDouble & DOUBLE_EXPONENT) != DOUBLE_SPECIAL)
        return assert(
            rawDouble != DOUBLE_NEGATIVE_ZERO || isCdata,
            "Number constant is negative zero",
            bytecode.filePath,
            DEBUG_INFO
        );
...
Ast::CONSTANT_TYPE Ast::get_constant_type(Expression* const& expression) {
    static const auto is_valid_number_constant = [](const double& number) -> bool {
        ...
        return (rawDouble & DOUBLE_EXPONENT) == DOUBLE_SPECIAL ? !(rawDouble & DOUBLE_FRACTION)
                                                               : rawDouble != DOUBLE_NEGATIVE_ZERO;
    };
    ...
    switch (expression->type) {
        ...
        case AST_EXPRESSION_BINARY_OPERATION:
            switch (expression->binaryOperation->type) {
                ...
                case AST_BINARY_MODULO:
                    if (get_constant_type(expression->binaryOperation->leftOperand) != NUMBER_CONSTANT ||
                        get_constant_type(expression->binaryOperation->rightOperand) != NUMBER_CONSTANT)
                        break;
                    double number;

                    switch (expression->binaryOperation->type) {
                        case AST_BINARY_ADDITION:
                            number = expression->binaryOperation->leftOperand->constant->number +
                                     expression->binaryOperation->rightOperand->constant->number;
                            break;
                        case AST_BINARY_SUBTRACTION:
                            number = expression->binaryOperation->leftOperand->constant->number -
                                     expression->binaryOperation->rightOperand->constant->number;
                        ...
                        case AST_BINARY_EXPONENTATION:
                            number = std::pow(
                                expression->binaryOperation->leftOperand->constant->number,
                                expression->binaryOperation->rightOperand->constant->number
                            );
                            break;
                        case AST_BINARY_MODULO:
                    ...
                    if (is_valid_number_constant(number))
                        return NUMBER_CONSTANT;
            }
        ...
        case AST_EXPRESSION_UNARY_OPERATION:
            switch (expression->unaryOperation->type) {
                case AST_UNARY_NOT:
                    if (get_constant_type(expression->unaryOperation->operand))
                        return BOOL_CONSTANT;
                    break;
                case AST_UNARY_MINUS:
                    if (expression->unaryOperation->operand->type == AST_EXPRESSION_CONSTANT) {
                        switch (expression->unaryOperation->operand->constant->type) {
                            case AST_CONSTANT_NUMBER:
                                if (!is_valid_number_constant(-expression->unaryOperation->operand->constant->number))
                                    break;
...
Ast::Expression* Ast::new_table(const Function& function, const uint16_t& index) {
    ...
    if (minimizeDiffs) {
        ...
        for (uint32_t i = function.get_constant(index).table.size(); i--;) {
            ...
            switch (key->constant->type) {
                ...
                case AST_CONSTANT_HASH:
                    ...
                    while (position && (hashFields[position - 1].key->constant->hashType > key->constant->hashType ||
                                        hashFields[position - 1].key->constant->hash > key->constant->hash)) {
                        position--;
                    }
                ...
                case AST_CONSTANT_STRING:
                    ...
                    while (position &&
                           stringFields[position - 1].key->constant->string.compare(key->constant->string) > 0) {
                        position--;
                    }
        ...
        if (falseField.key)
            expression->table->constants.fields.emplace_back(falseField);
        if (trueField.key)
            expression->table->constants.fields.emplace_back(trueField);
        expression->table->constants.fields.reserve(
            expression->table->constants.fields.size() + numberFields.size() + hashFields.size() + stringFields.size()
        );
        expression->table->constants.fields.insert(
            expression->table->constants.fields.begin() + expression->table->constants.fields.size(),
            numberFields.begin(),
            numberFields.begin() + numberFields.size()
        );
        expression->table->constants.fields.insert(
            expression->table->constants.fields.begin() + expression->table->constants.fields.size(),
            hashFields.begin(),
            hashFields.begin() + hashFields.size()
        );
        expression->table->constants.fields.insert(
            expression->table->constants.fields.begin() + expression->table->constants.fields.size(),
            stringFields.begin(),
            stringFields.begin() + stringFields.size()
        );
    } else {
        ...
        for (uint32_t i = expression->table->constants.fields.size(); i--;) {
            ...
            if (expression->table->constants.fields[i].key->constant->type == AST_CONSTANT_STRING)
                check_valid_name(expression->table->constants.fields[i].key->constant);
            expression->table->constants.fields[i].value =
                new_table_constant(function.get_constant(index).table[i].value);
        }
    }
