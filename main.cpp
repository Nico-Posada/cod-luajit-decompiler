...
static std::vector<std::filesystem::path> find_input_files(const std::filesystem::path& inputPath, const std::filesystem::path& outputPath) {
...
static bool decompile_file(const std::filesystem::path& inputFile, const std::filesystem::path& outputFile) {
	...
	Lua lua(bytecode, ast, outputFile.string(), arguments.forceOverwrite, arguments.minimizeDiffs, arguments.unrestrictedAscii);
...
int main(int argc, char* argv[]) try {
	...
	const std::filesystem::path outputPath = std::filesystem::absolute(
		arguments.outputPath.empty()
			? (inputIsDirectory ? inputPath : inputPath.parent_path()) / "output"
	...
	for (const std::filesystem::path& inputFile : inputFiles) {
		...
		if (!decompile_file(inputFile, outputPath / outputFile)) filesSkipped++;
	}
