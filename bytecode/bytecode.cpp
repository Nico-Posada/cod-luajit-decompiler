...
void Bytecode::read_header() {
    read_file(5);
    assert(
        fileBuffer[0] == BC_HEADER[0] && fileBuffer[1] == BC_HEADER[1] && fileBuffer[2] == BC_HEADER[2],
        "Invalid header:\nExpected bytes " + byte_to_string(BC_HEADER[0]) + " " + byte_to_string(BC_HEADER[1]) + " " +
            byte_to_string(BC_HEADER[2]) + ", got " + byte_to_string(fileBuffer[0]) + " " +
            byte_to_string(fileBuffer[1]) + " " + byte_to_string(fileBuffer[2]) +
            "\n\nFile does not contain valid LuaJIT bytecode",
        filePath,
        DEBUG_INFO
    );
    header.version = fileBuffer[3];
    assert(
        header.version == BC_VERSION_82 || header.version == BC_VERSION_83 || header.version == BC_VERSION_84,
        "Invalid bytecode version (" + byte_to_string(fileBuffer[3]) + ")",
        filePath,
        DEBUG_INFO
    );
    header.flags = fileBuffer[4];
    assert(
        !(header.flags & ~(BC_F_BE | BC_F_STRIP | BC_F_FFI | BC_F_FR2 | BC_F_STRIP_VAR)),
        "Invalid flags (" + byte_to_string(header.flags) + ")",
        filePath,
        DEBUG_INFO
    );
    assert(!(header.flags & BC_F_BE), "Big endian support not implemented", filePath, DEBUG_INFO); // TODO
    if (header.flags & BC_F_STRIP || header.flags & BC_F_STRIP_VAR)
        return;
...
void Bytecode::read_prototypes() {
    ...
    assert(
        (main->header.flags & BC_PROTO_VARARG) && !main->header.parameters && !main->upvalues.size(),
...
void Bytecode::open_file() {
    ...
    assert(fileSize >= 0, "Unable to determine file size", filePath, DEBUG_INFO);
    assert(fileSize >= MIN_FILE_SIZE, "File is too small or empty", filePath, DEBUG_INFO);
    bytesUnread = static_cast<uint64_t>(fileSize);
    file.seekg(0);
}
