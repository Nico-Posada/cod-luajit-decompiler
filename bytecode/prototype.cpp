...
void Bytecode::Prototype::operator()(std::vector<Prototype*>& unlinkedPrototypes) {
    ...
    read_debug_info();
    assert(
        prototypeSize == bytecode.fileBuffer.size(), "Prototype has unread bytes left", bytecode.filePath, DEBUG_INFO
    );
...
void Bytecode::Prototype::read_header() {
    ...
    assert(
        !(header.flags & ~(BC_PROTO_CHILD | BC_PROTO_VARARG | BC_PROTO_FFI)),
        "Prototype has invalid flags (" + byte_to_string(header.flags) + ")",
        bytecode.filePath,
        DEBUG_INFO
    );
    ...
    assert(
        constantsSize <= bytecode.fileBuffer.size() - prototypeSize,
        "Prototype constant count exceeds remaining bytes",
        bytecode.filePath,
        DEBUG_INFO
    );
    ...
    assert(
        numberConstantsSize <= bytecode.fileBuffer.size() - prototypeSize,
        "Prototype number constant count exceeds remaining bytes",
        bytecode.filePath,
        DEBUG_INFO
    );
    ...
    assert(
        static_cast<uint64_t>(instructionsSize) * 4 <= bytecode.fileBuffer.size() - prototypeSize,
        "Prototype instruction count exceeds remaining bytes",
        bytecode.filePath,
        DEBUG_INFO
    );
    instructions.resize(instructionsSize);
    assert(instructions.size(), "Prototype has no instructions", bytecode.filePath, DEBUG_INFO);
    if (bytecode.header.flags & BC_F_STRIP || !get_uleb128())
        return;
...
void Bytecode::Prototype::read_instructions() {
    for (uint32_t i = 0; i < instructions.size(); i++) {
        ...
        assert(
            ...
            "Prototype has invalid instruction (" + byte_to_string(instructions[i].type) + ")",
            bytecode.filePath,
            DEBUG_INFO
        );
        ...
        switch (instructions[i].type) {
            ...
            case BC_OP_FUNCCW:
                assert(
                    false,
                    "Prototype has unsupported instruction (" + byte_to_string(instructions[i].type) + ")",
                    bytecode.filePath,
                    DEBUG_INFO
                );
        }
    ...
    for (uint32_t i = 0; i < constants.size(); i++) {
        ...
        switch (type) {
            ...
            case BC_KGC_TAB: {
                ...
                assert(
                    static_cast<uint64_t>(arraySize) + static_cast<uint64_t>(tableSize) * 2 <=
                        bytecode.fileBuffer.size() - prototypeSize,
                    "Prototype table exceeds remaining bytes",
                    bytecode.filePath,
                    DEBUG_INFO
                );
            ...
            case BC_KGC_COMPLEX:
                assert(
                    !get_uleb128() && !get_uleb128(),
                    "Prototype has invalid cdata constant",
                    bytecode.filePath,
                    DEBUG_INFO
                );
            case BC_KGC_I64:
            ...
            default:
                ...
                assert(
                    type - BC_KGC_STR <= bytecode.fileBuffer.size() - prototypeSize,
                    "Prototype string exceeds remaining bytes",
                    bytecode.filePath,
                    DEBUG_INFO
                );
...
void Bytecode::Prototype::read_debug_info() {
    ...
    assert(
        parameterCount == header.parameters,
        "Prototype parameter count does not\nmatch with debug info",
        bytecode.filePath,
        DEBUG_INFO
    );
...
uint8_t Bytecode::Prototype::get_next_byte() {
    assert(
        prototypeSize < bytecode.fileBuffer.size(),
...
Bytecode::TableConstant Bytecode::Prototype::get_table_constant() {
    ...
    switch (type) {
        ...
        default:
            ...
            assert(
                type - BC_KTAB_STR <= bytecode.fileBuffer.size() - prototypeSize,
                "Prototype table string exceeds remaining bytes",
                bytecode.filePath,
                DEBUG_INFO
            );
