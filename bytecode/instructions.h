...
enum BC_OP {
    ...
    BC_OP_HGGET,  // A<DST> = _G[D<HASH>]
    BC_OP_HGSET,  // _G[D<HASH>] = A<VAR>
    ...
    BC_OP_CALLM,  // if B<LIT> == 0 then A<BASE> (<- multres) <- A(A+FR2?2:1, A++ -> for C<LIT>, A++ (<- multres)) else
                  // A, A++ -> for B-1 = A(A+FR2?2:1, A++ -> for C, A++ (<- multres))
    BC_OP_CALL, // if B<LIT> == 0 then A<BASE> (<- multres) <- A(A+FR2?2:1, A++ -> for C-1<LIT>) else A, A++ -> for B-1
                // = A(A+FR2?2:1, A++ -> for C-1)
...
static BC_OP get_op_type(const uint8_t& byte, const uint8_t& version) {
    return (BC_OP)(version != Bytecode::BC_VERSION_84 && byte >= BC_OP_HGGET ? byte + 2 : byte);
}
